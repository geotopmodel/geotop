				  ...
				  
{
	...
	if(par->Dtplot_point->co[i_sim] > 1.E-5){
		...
		if(par->state_pixel == 1){
			...
			for(i=1;i<=par->rc->nrh;i++){
				...
				if (D < 1.E-5 && sl->SS->P->co[0][j] > 0) D = -sl->SS->P->co[0][j] / cos(top->slope->co[r][c] * Pi/180.);
		...
		if (fabs(t_point - par->Dtplot_point->co[i_sim])<1.E-5){
			...
			if(par->state_pixel == 1){
				...
				for(i=1;i<=par->rc->nrh;i++){
					...
					//sl output
					write_soil_output(i, par->IDpoint->co[i], par->init_date->co[i_sim], JDfrom0, JD, day, month, year, hour, minute, par->soil_plot_depths, sl, par, PsiMin, cosslope);
	...
	if(par->output_soil->co[i_sim]>0 && fmod(times->time+par->Dt,par->output_soil->co[i_sim]*3600.0)<1.E-5){
		...
		if( strcmp(files[fwtable_up] , string_novalue) != 0 ){
			...
			for(i=1; i<=par->total_pixel; i++){
				...
				if (V->co[i] < 1.E-5 && sl->SS->P->co[0][i] > 0) V->co[i] = -sl->SS->P->co[0][i] / cos(top->slope->co[r][c] * Pi/180.);
			}
		...
		if( strcmp(files[fwtable_dw] , string_novalue) != 0 ){
			...
			for(i=1; i<=par->total_pixel; i++){
				...
				c = top->rc_cont->co[i][2];
				if (sl->SS->P->co[0][i] > 0){
					V->co[i] = -sl->SS->P->co[0][i] / cos(top->slope->co[r][c] * Pi/180.);
				}else {
	...
	/**********************************************************************************************************/
	/**********************************************************************************************************/
	//printf("point_sim=%d, isavings=%ld, par->saving_points->nh=%ld, par->delay_day_recover=%f",par->point_sim,isavings,par->saving_points->nh,par->delay_day_recover); stop_execution();
...
void write_soil_output(long i, long iname, double init_date, double JDfrom0, double JD, long day, long month, long year, long hour, long minute, DOUBLEVECTOR *n, SOIL *sl, PAR *par, double psimin, double cosslope){
	...
	if(strcmp(files[fTz] , string_novalue) != 0){
		...
		f=fopen(name,"a");	
		write_soil_file(1, iname, f, day, month, year, hour, minute, JDfrom0, init_date, sl->Tzplot->co[i], n, sl->pa->co[1][jdz], cosslope);
		fclose(f);
		free(name);
		free(temp);
	}
	...
	if(strcmp(files[fTzwriteend] , string_novalue) != 0){
		write_soil_file(1, iname, ffT, day, month, year, hour, minute, JDfrom0, init_date, sl->Tzplot->co[i], n, sl->pa->co[1][jdz], cosslope);
	}
	...
	if(strcmp(files[fTzav] , string_novalue) != 0){
		...
		f=fopen(name,"a");	
		write_soil_file(1, iname, f, day, month, year, hour, minute, JDfrom0, init_date, sl->Tzavplot->co[i], n, sl->pa->co[1][jdz], cosslope);
		fclose(f);
		free(name);
		free(temp);
	}
	...
	if(strcmp(files[fTzavwriteend] , string_novalue) != 0){
		write_soil_file(1, iname, ffTav, day, month, year, hour, minute, JDfrom0, init_date, sl->Tzavplot->co[i], n, sl->pa->co[1][jdz], cosslope);
	}
	...
	if(strcmp(files[fpsiztot] , string_novalue) != 0){
		...
		f=fopen(name,"a");		
		write_soil_file(1, iname, f, day, month, year, hour, minute, JDfrom0, init_date, sl->Ptotzplot->co[i], n, sl->pa->co[1][jdz], cosslope);
		fclose(f);
		free(name);
		free(temp);
	}
	...
	if(strcmp(files[fpsiztotwriteend] , string_novalue) != 0){
		write_soil_file(1, iname, ffpsitot, day, month, year, hour, minute, JDfrom0, init_date, sl->Ptotzplot->co[i], n, sl->pa->co[1][jdz], cosslope);
	}
	...
	if(strcmp(files[fpsiz] , string_novalue) != 0){
		...
		f=fopen(name,"a");		
		write_soil_file(0, iname, f, day, month, year, hour, minute, JDfrom0, init_date, sl->Pzplot->co[i], n, sl->pa->co[1][jdz], cosslope);
		fclose(f);
		free(name);
		free(temp);
	}
	...
	if(strcmp(files[fpsizwriteend] , string_novalue) != 0){
		write_soil_file(0, iname, ffpsi, day, month, year, hour, minute, JDfrom0, init_date, sl->Pzplot->co[i], n, sl->pa->co[1][jdz], cosslope);
	}
	...
	if(strcmp(files[fliqz] , string_novalue) != 0){
		...
		f=fopen(name,"a");		
		write_soil_file(1, iname, f, day, month, year, hour, minute, JDfrom0, init_date, sl->thzplot->co[i], n, sl->pa->co[1][jdz], cosslope);
		fclose(f);
		free(name);
		free(temp);
	}
	...
	if(strcmp(files[fliqzwriteend] , string_novalue) != 0){
		write_soil_file(1, iname, ffliq, day, month, year, hour, minute, JDfrom0, init_date, sl->thzplot->co[i], n, sl->pa->co[1][jdz], cosslope);
	}
	...
	if(strcmp(files[fliqzav] , string_novalue) != 0){
		...
		f=fopen(name,"a");		
		write_soil_file(1, iname, f, day, month, year, hour, minute, JDfrom0, init_date, sl->thzavplot->co[i], n, sl->pa->co[1][jdz], cosslope);
		fclose(f);
		free(name);
		free(temp);
	}
	...
	if(strcmp(files[fliqzavwriteend] , string_novalue) != 0){
		write_soil_file(1, iname, ffliqav, day, month, year, hour, minute, JDfrom0, init_date, sl->thzavplot->co[i], n, sl->pa->co[1][jdz], cosslope);
	}
	...
	if(strcmp(files[ficez] , string_novalue) != 0){
		...
		f=fopen(name,"a");		
		write_soil_file(1, iname, f, day, month, year, hour, minute, JDfrom0, init_date, sl->thizplot->co[i], n, sl->pa->co[1][jdz], cosslope);
		fclose(f);
		free(name);
		free(temp);
	}
	...
	if(strcmp(files[ficezwriteend] , string_novalue) != 0){
		write_soil_file(1, iname, ffice, day, month, year, hour, minute, JDfrom0, init_date, sl->thizplot->co[i], n, sl->pa->co[1][jdz], cosslope);
	}
	...
	if(strcmp(files[ficezav] , string_novalue) != 0){
		...
		f=fopen(name,"a");		
		write_soil_file(1, iname, f, day, month, year, hour, minute, JDfrom0, init_date, sl->thizavplot->co[i], n, sl->pa->co[1][jdz], cosslope);
		fclose(f);
		free(name);
		free(temp);
	}
	...
	if(strcmp(files[ficezavwriteend] , string_novalue) != 0){
		write_soil_file(1, iname, fficeav, day, month, year, hour, minute, JDfrom0, init_date, sl->thizavplot->co[i], n, sl->pa->co[1][jdz], cosslope);
	}
