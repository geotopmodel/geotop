...
long ncgt_add_output_var_par(int ncid, size_t start, size_t count, void *m, double time, short nlimdim, const char* dimension_time,const char* dimension_z,const char* dimension_x,
	 ...
	 *
	 * \param ncid -  (int) pointer to the netCDF archive file
	 * \param start - (size_t) vector of size_t integers specifying the index in the variable where the first of the data values will be written
	 * \param count - (size_t) vector of size_t integers specifying the edge lengths along each dimension of the block of data values to be written
	 * \param m - (void *) variable to be printed (can be doublematrix, doublevector, doubletensor)
	 * \param dimension_time
	 * \param dimension_z - (char *) vertical dimension
	 * \param dimension_y - (char *) dimension 1
