...
if(NN==1){	for(i=1;i<v->nh;i++){
		    
			if(v->co[i]!=novalue){ 
				moment+=v->co[i];
	        	n++;
	        }	}
moment/=n;}else if(NN==2) {
...
moment=(moment/n-mean*mean);}else {	
	for(i=1;i<v->nh;i++){
...
} else if(m->nrh <1 || m->nch <1 || m->isdynamic !=1 || s->nrh <1 || s->nch <1 || s->isdynamic !=1) {
	t_error("thess matrices was not properly allocated");}else if(m->nrh !=s->nrh || m->nch!=s->nch){
