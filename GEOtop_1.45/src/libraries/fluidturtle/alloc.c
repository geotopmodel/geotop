...

{
	...
	if (!v)
		t_error("allocation failure in fvector()");
...

{
	...
	if (!m)
		t_error("Allocation failure 1 in matrix()");
	...
	m[nrl]
			= (float *) malloc(
					(size_t) ((rows * cols + NR_END) * sizeof(float)));

	if (!m)
		t_error("Allocation failure 2 in matrix()");
...

{
	...
	m[nrl]
			= (short *) malloc(
					(size_t) ((rows * cols + NR_END) * sizeof(short)));
...

{
	...
	m[nrl] = (double *) malloc((size_t) ((rows * cols + NR_END)
			* sizeof(double)));
...

{
	...
	if (!m)
		t_error("allocation failure in SHORTVECTOR()");
...

{
	...
	if (!m)
		t_error("allocation failure in INTVECTOR()");
...

{
	...
	if (!m)
		t_error("allocation failure in FLOATVECTOR()");
...

{
	...
	if (!m)
		t_error("allocation failure in LONGVECTOR()");
...

{
	...
	if (!m)
		t_error("allocation failure in DOUBLEVECTOR()");
...

{
	...
	if (!m)
		t_error("allocation failure in CHARVECTOR()");
...

{
	...
	if (!m)
		t_error("allocation failure in SHORTMATRIX()");
...

{
	...
	if (!m)
		t_error("allocation failure in INTMATRIX()");
...

{
	...
	if (!m)
		t_error("allocation failure in floatmatrix()");
...

{
	...
	if (!m)
		t_error("allocation failure in LONGMATRIX()");
...

{
	...
	if (!m)
		t_error("allocation failure in new_doublematrix()");
...

{
	...
	if (!l)
		t_error("allocation failure in new_intbin()");
	...
	if (indx->nl != 1) {
		...
		if (!(l->co[1]))
			t_error("allocation failure 2 in new_intbin()");
...

{
	...
	if (!l)
		t_error("allocation failure in new_shortbin()");
	...
	if (indx->nl != 1) {
		...
		l->co = (short **) malloc((size_t) ((indx->nh + NR_END)
				* sizeof(short *)));
		...
		if (!(l->co[1]))
			t_error("allocation failure 2 in new_shortbin()");
...

{
	...
	if (!l)
		t_error("allocation failure in new_longbin()");
	...
	if (indx->nl != 1) {
		...
		l->co = (long **) malloc(
				(size_t) ((indx->nh + NR_END) * sizeof(long *)));
		...
		if (!(l->co[1]))
			t_error("allocation failure 2 in new_longbin()");
...

{
	...
	if (!l)
		t_error("allocation failure in new_doublebin()");
	...
	if (indx->nl != 1) {
		...
		l->co = (double **) malloc((size_t) ((indx->nh + NR_END)
				* sizeof(double *)));
		...
		l->co[1]
				= (double *) malloc((size_t) ((sum + NR_END) * sizeof(double)));

		if (!(l->co[1]))
			t_error("allocation failure 2 in new_doublebin()");
...

{
	...
	if (!l)
		t_error("allocation failure in new_stringcontainer()");
	...
	if (indx->nl != 1) {
		...
		l->co = (char **) malloc(
				(size_t) ((indx->nh + NR_END) * sizeof(char *)));
		...
		if (!(l->co[1]))
			t_error("allocation failure 2 in new_stringcontainer()");
...

{
	...
	} else if (l->index == NULL || (l->index)->nl != 1 || (l->index)->nl
			> (l->index)->nh ||
