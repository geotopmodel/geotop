...
{
long i,sum=0;STRINGBIN *l;
    l=(STRINGBIN *)malloc(sizeof(STRINGBIN));		  if (!l) t_error("allocation failure in new_stringcontainer()");
	l->isdynamic=isDynamic;			if(indx->nl!=1){		t_error("Not a proper index for stringcontainer");	}else{		l->index=new_longvector(indx->nh);		
		(l->index)->nl=1;
		(l->index)->nh=indx->nh;
			
		for(i=indx->nl;i<=indx->nh;i++){
			(l->index)->co[i]=indx->co[i];				}		   		l->co=(char **)malloc((size_t) ((indx->nh+NR_END)*sizeof(char *)));		if (!l->co) t_error("allocation failure in new_stringcontainer()");		l+=NR_END-1;
		for(i=1;i<=indx->nh; i++){			sum+=(l->index)->co[i];
		}						l->co[1]=(char *)malloc((size_t) ((sum+NR_END)*sizeof(char)));		if(!(l->co[1])) t_error("allocation failure 2 in new_stringcontainer()");		l->co[1]+=NR_END;		l->co[1]-=1;				for(i=2;i<=indx->nh; i++){			l->co[i]=l->co[i-1]+indx->co[i-1];		}			}l->next=NULL;
return l;
}/*------------------------------------------------------------------------
