...
{	
	...
	for(i=0;i<=5;i++){	
		...
		if(ch[0]==45){
			...
			if(ch[0]<=45 || ch[0]==47 || ch[0]>=58){
				printf("Error: - not followed by a number for r=%ld c=%ld, file %s",r,c,name);
				t_error("Fatal error");
			}
		}
		if(ch[0]>=48 && ch[0]<=57){		
	...
	for(r=1;r<=nr;r++){
			...
			do{
				...
				if(ch[0]==10){	//end of line
					if(c==nc+1 || r>nr){
						...
						printf("Number of cols less than declared in row %ld in file %s, c:%ld",r,join_strings(name,ascii_grass),c);
						t_error("Fatal error");
					}
				}
		...
		if(c<nc && r<=nr){
			printf("Number of cols less than declared in row %ld in file %s, c:%ld nc:%ld r:%ld nr:%ld",r,join_strings(name,ascii_grass),c,nc,r,nr);
			t_error("Fatal error");
		}
		if(c>nc && r<=nr){
...
{	
	...
	for(i=0;i<=4;i++){	
		...
		if(ch[0]==45){
			...
			ch[0]=fgetc(f);
			if(ch[0]<=45 || ch[0]==47 || ch[0]>=58){
				printf("Error: +- not followed by a number for r=%ld c=%ld, file %s",r,c,temp);
				t_error("Fatal error");
			}
		}
		if(ch[0]>=48 && ch[0]<=57){
	...
	if(header[0]<=0 || header[1]<=0){
		printf("In file %s nrows or ncols negative or null %ld %ld",temp, (long)header[0], (long)header[1]);
		t_error("Fatal error");
	}	
	...
	if(ch[0]==78 || ch[0]==110){	//character "N" or "n", NODATA_value
		...
		for(r=1;r<=nr;r++){
				...
				do{
					...
					if(ch[0]==10){	//end of line
						if(c==nc+1 || r>nr){
							...
							printf("Number of cols less than declared in row %ld in file %s c:%ld",r,temp,c);
							t_error("Fatal error");
						}
					}
					if(ch[0]==-1){	//end of file
			...
			if(c<nc && r<=nr){
				printf("Number of cols less than declared in row %ld in file %s, c:%ld nc:%ld r:%ld nr:%ld",r,temp,c,nc,r,nr);
				t_error("Fatal error");
			}
			if(c>nc && r<=nr){
