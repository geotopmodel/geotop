...
for (kj=vname->nl;kj<=nrows;kj++){
	vname->co[kj]=stringarray->index->co[2*kj-1];
	vcomment->co[kj]=stringarray->index->co[2*kj];
//	printf("\n vcom= %d \n",vcomment->co[kj]);
}
...
for (kj=1;kj<=nrows;kj++){
	...
	strcpy(keywords->comments->co[kj]+1,stringarray->co[2*kj]+1);
	strcpy(keywords->names->co[kj]+1,stringarray->co[2*kj-1]+1);
...
/*!
	...
	* \relate FLUIDTURLE_STATICLIBRARY.read_stringarray
    ...
    nl=NL;
	nh=index;
    array->co=(KEYWORDS **)malloc((size_t)((nh-nl+1+NR_END)*sizeof(KEYWORDS *)));
    array->isdynamic=isDynamic;
    array->nh=nh;
    array->nl=nl;

    for (j=nl;j<=nh;j++){
    	array->co[j]=read_keywords(fd,print);
    }
 ...
 */
    ...
    for (kcount=keywords->names->index->nl;kcount<=keywords->names->index->nh;kcount++){
		  ...
		  printf ("\nkeyword n %ld : %s  as  %s \n ",kcount,keywords->names->co[kcount]+1,keywords->comments->co[kcount]+1);
	...
	*/
	 ...
	 if (filenames->index->nh!=comments->index->nh) {
		 ...
		 for (kcount=filenames->index->nl;kcount<=filenames->index->nh;kcount++){
			 printf ("\nREAD FILENAME: %s as file %ld  i.e. %s",filenames->co[kcount]+1,kcount,comments->co[kcount]+1);
...
tempo troppo alto
		 ...
		 \param jread          - (long) - the number releted to the co for each KEYWORDS struct contained in keywords_model which contains the model keyword
		 ...
		 \details The variable keyword_model contains two or more KEYWORDS struct. In each of these the jread-th co is the keyword of the model component (normally jread is assumed equal to 1), the other elements all possible values of the model component. Here the order of the model component is here modifed according to  the order of the keywords in written_palette.
    ...
    palette->co=(KEYWORDS **)malloc((size_t)((keywords_model->index->nh-keywords_model->index->nl+1+NR_END)*sizeof(KEYWORDS *)));
	...
	for (keycnt=keywords_model->index->nl;keycnt<=keywords_model->index->nh;keycnt++){
		...
		key_ref=keywords_model->co[keycnt]+1;

		lennames->co[keycnt]=(long)strlen(empty_name);
		keyreferences->co[keycnt]=EMPTY_VALUE;

		for (namescnt=written_palette->nl;namescnt<=written_palette->nh;namescnt++){

			key_written=written_palette->co[namescnt]->names->co[jread]+1;

			if (!strcmp(key_ref,key_written)) keyreferences->co[keycnt]=namescnt;
	...
	for (keycnt=keywords_model->index->nl;keycnt<=keywords_model->index->nh;keycnt++){
		...
		palette->co[keycnt]=(KEYWORDS *)malloc(sizeof(KEYWORDS *));
		if (!palette->co[keycnt]) t_error("Keywords palette->co[j]  was not allocated");

		if (keyreferences->co[keycnt]==EMPTY_VALUE) {
			vi=new_longvector(2);
			wi=new_longvector(2);

			vi->co[1]=(long)strlen(keywords_model->co[keycnt]+1)+1;
			vi->co[2]=(long)strlen(empty_name)+1;
			wi->co[1]=(long)strlen(empty_name)+1;
			wi->co[2]=(long)strlen(empty_name)+1;

			palette->co[keycnt]->names=new_stringbin(vi);
			palette->co[keycnt]->comments=new_stringbin(wi);

			strcpy(palette->co[keycnt]->names->co[1]+1,keywords_model->co[keycnt]+1);
			strcpy(palette->co[keycnt]->comments->co[1]+1,empty_name);

			strcpy(palette->co[keycnt]->names->co[2]+1,empty_name);
			strcpy(palette->co[keycnt]->comments->co[2]+1,empty_name);
			...
			palette->co[keycnt]->names=new_stringbin(written_palette->co[keyreferences->co[keycnt]]->names->index);
			palette->co[keycnt]->comments=new_stringbin(written_palette->co[keyreferences->co[keycnt]]->comments->index);

			for (j=palette->co[keycnt]->names->index->nl;j<=palette->co[keycnt]->names->index->nh;j++){
				strcpy(palette->co[keycnt]->names->co[j]+1,written_palette->co[keyreferences->co[keycnt]]->names->co[j]+1);
			    strcpy(palette->co[keycnt]->comments->co[j]+1,written_palette->co[keyreferences->co[keycnt]]->comments->co[j]+1);
...
void free_keywords_list(KEYWORDS_LIST* keywords_list){
	...
	for (jj=keywords_list->nl;jj<=keywords_list->nh;jj++) {
		free_keywords(keywords_list->co[jj]);
	}
...
/*!
 ...
 /*!<
	 ...
	 for (keycnt=keywords->index->nl; keycnt<=keywords->index->nh;keycnt++) {
		 ...
		 key_ref=keywords->co[keycnt]+1;
		 empty_name_extended=join_strings(empty_name_temp,keywords->co[keycnt]+1);
		 lennames->co[keycnt]=(long)strlen(empty_name_extended)+1;
		 ...
		 for (namescnt=written_names->comments->index->nl;namescnt<=written_names->comments->index->nh;namescnt++){
			 ...
			 key_written=written_names->comments->co[namescnt]+1;
//			 printf ("\n keycnt= %d namescnt= %d  \n ",keycnt,namescnt);
//			 printf(key_ref);
//			 printf("\n and \n");
//			 printf(key_written);
//			 printf("\n");
			 ...
			 if (!strcmp(key_ref,key_written))   {
				...
				lennames->co[keycnt]=written_names->names->index->co[namescnt];
				keyreferences->co[keycnt]=namescnt;
	 ...
	 for (keycnt=names->index->nl;keycnt<=names->index->nh;keycnt++){
		 if (keyreferences->co[keycnt]==EMPTY_VALUE) {
			 strcpy(names->co[keycnt]+1,empty_name);
			 //empty_name_extended=join_strings(empty_name_temp,keywords->co[keycnt]+1);
			 //strcpy(names->co[keycnt]+1,empty_name_extended);
			 //free(empty_name_extended);
			 
		 }else  {
			 strcpy(names->co[keycnt]+1,written_names->names->co[keyreferences->co[keycnt]]+1);
		 }
 ...
 \brief It adds a string to each co of a stringbin
 \author Emanuele Cordano
 ...
 */
 ...
 for (i=lenstring->nl;i<=lenstring->nh;i++) {
	 ...
	 if (strncmp(stringvector->co[i]+1,"#",1) == 0){
 		//if current line start with # -> netcdf variable definition
		 lenstring->co[i]=(long)strlen(stringvector->co[i]+1)+1;
	}else if (!strcmp(stringvector->co[i]+1,no_joinstring)) {
#else
 	if (!strcmp(stringvector->co[i]+1,no_joinstring)) {
#endif
 		lenstring->co[i]=(long)strlen(stringvector->co[i]+1)+1;
 	} else {
 		str=join_strings(path,stringvector->co[i]+1);
 		lenstring->co[i]=(long)strlen(str)+1;
 		free(str);
 	}
	//printf("%ld-%s\n",i,stringvector->co[i]+1);
 }
 ...
 for (i=lenstring->nl;i<=lenstring->nh;i++) {
	 ...
	 if (strncmp(stringvector->co[i]+1,"#",1) == 0){
	 	...
	 	strcpy(joinedstring->co[i]+1,stringvector->co[i]+1);
	}else if (!strcmp(stringvector->co[i]+1,no_joinstring)) {
#else
	if (!strcmp(stringvector->co[i]+1,no_joinstring)) {
#endif
 		...
 		str=join_strings(path,stringvector->co[i]+1);
 		strcpy(joinedstring->co[i]+1,str);
 		free(str);
 	}
	//printf("%ld-%s-%s\n",i,stringvector->co[i]+1,joinedstring->co[i]+1);
 }
