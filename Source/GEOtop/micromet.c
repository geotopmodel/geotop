...
However, he asks to give credit to Liston and Elder, 2006, when using it with satisfaction. 
...
//   relative humidity.
	...
	//Convert again the measurements from Tdew to relative humidity
	for(n=1;n<=met->st->Z->nh;n++){
		colRH=met->column[n-1][RHcode];
		colT=met->column[n-1][Tcode];
        if(colRH!=-1){
			if(met->var[n-1][colRH]!=novalue){
				...
				// Convert to actual altitude data from sea level values.
				delta_topo = topo_ref - met->st->Z->co[n];
				met->var[n-1][colRH] -= (1.E-3*Td_lapse_rate * delta_topo + tk);

				//convert Tdew in RH
				met->var[n-1][colRH] = 100.0*RHfromTdew(met->var[n-1][colT], met->var[n-1][colRH], met->st->Z->co[n]);
...
// Convert the length scale to an appropriate grid increment.
	inc = (long)(Fmax(1.0, curve_len_scale/deltaxy));
