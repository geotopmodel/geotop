...
void BlowingSnow(long r, long c, float FetchDist, SNOW *snow, double T, double RH, double Uten, double StubHt, double *Transport, double *Sublimation, double *LatentH){
	...
	if (EffectiveStubHt > 0.01){
		Znod = (pow(Ustar_main, (float)2.)/163.3) + 0.5*320*EffectiveStubHt*0.003; //Eq. 29, Snowcover Book
...
void set_windtrans_snow(SNOW *snow, METEO *met, LAND *land, PAR *par, double t){
	...
	for(r=1;r<=nr;r++){
		for(c=1;c<=nc;c++){
			if(land->LC->co[r][c]!=NoV){
				Dsum += DEPTH(r,c,snow->lnum,snow->Dzl)/(double)par->total_pixel;
			}
		}
	}
			...
			for(c=1;c<=nc;c++){
				if(land->LC->co[r][c]!=NoV){
					...
					if(D > land->ty->co[lu][jz0thressoil]){
						...
						if( par->vegflag->co[lu]==1 ) meteo_interp2(&line_interp, land->vegparv[lu], land->vegparp->co[lu], jdvegprop, 1, 0, land->vegpars[lu], t+0.5*par->Dt, par->JD0, par->year0, NoV);
						if( land->vegparv[lu][land->vegparp->co[lu][jdHveg]] != NoV ){
							StubHt = 1.E-3*(land->vegparv[lu][land->vegparp->co[lu][jdHveg]] - D);
						}else{
							StubHt = 1.E-3*(land->ty->co[lu][jHveg] - D);
						}
					}
					...
					BlowingSnow(r, c, FetchDist, snow, T, RH, par->incr_V_factor*U, StubHt, &(snow->Qtrans->co[r][c]), &(snow->Qsub->co[r][c]), &LatH);
	...
	if( (Qsubl>0 || Qtrans>0) && Dsum>0){
		...
		extend_topography_row(snow->Qtrans, NoV);
		...
		set_no_value(snow->Qtrans, land->LC, NoV);	
		extend_topography_column(snow->Qtrans, NoV);
			...
			for(c=1;c<=nc;c++){
				if(land->LC->co[r][c]!=UV->V->co[2]){
					...
					for(l=snow->lnum->co[r][c];l>=1;l--){
						...
						snow->Dzl->co[l][r][c] *= Fmax(0.1, 1.0 + CR*par->Dt);	
					}
					...
					if(ns>0){	//snow on the soil
						...
						if(DW<0){	//snow eroded
							...
							/*from_internal_energy(par->alpha_snow, r, c, h + internal_energy(DW, 0.0, snow->T->co[i][r][c]), &(snow->w_ice->co[i][r][c]),
					...
					snow_layer_combination(par->alpha_snow, r, c, snow, T, par->snowlayer_inf, par->Dmin, par->Dmax, t);
