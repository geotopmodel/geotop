...
void aero_resistance2(double zmu, double zmt, double z0, double d0, double z0_z0t, double hveg, double v, double Ta, double T, 
					  double Qa, double Q, double P, double gmT, double LSAI, double *rm, double *rh, double *rv, double *u_top, 
					  double *Lobukhov, short state_turb, short MO, long maxiter){
...
void Lewis(double zmu, double zmt, double d0, double z0, double z0_z0t, double Ta, double Ts, double v, double *rm, double *rh, 
			...
			*beta = theta[1];
			...
			for(l=1;l<=n;l++){
				...
				if(l==1){
					...
					sat_spec_humidity_2(&Qsat, 1., T[l], P);
					r->co[l] += r->co[l-1];
				}
			...
			evap_layer->co[1] -= rho*(soil[jsat][1] - theta[1]) * Qs / r->co[1]; 
