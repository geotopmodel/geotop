...
/* KeyPalette MANAGES  THE I/O FILES AND COMPONENTS OF
KeyPalette Version 0.9375 Lavagna
 ...
 */
 ...
 for (i=lenstring->nl;i<=lenstring->nh;i++) {
 	if (!strcmp(stringvector->element[i]+1,no_joinstring)) {
 		...
 		lenstring->element[i]=(long)strlen(join_strings(path,stringvector->element[i]+1))+1;
 	}
 }
 ...
 for (i=lenstring->nl;i<=lenstring->nh;i++) {
 	if (!strcmp(stringvector->element[i]+1,no_joinstring)) {
 		...
 		strcpy(joinedstring->element[i]+1,join_strings(path,stringvector->element[i]+1));
 	}
