...
do{
		if(buffer_index < buffer_size){
			...
			if(buffer_size==maxbuffersize){
				...
				printf ("Warning::missing part of string %d and other %d strings\n",count,df);
...

{
    ...
    char buffer[64*FILENAME_MAX],*bf,*pathfile="$WorkingPath";
    long len,i;
	short a;
    FILE *istream;
...
char *fullname="\0",*pathfile="\0",*S=".inpts";
FILE *istream;
...
long buffer_index=0,blocksnumber=0,pos=0;
long blocks;
...
long buffer_index=0,buffer_size=0,blocksnumber=0,i, jj;
...
long strings_index=0,strings_size=0,blocksnumber=0,char_counter=0,strings_counter=0;
	...
	fprintf(outputfile,"%d: %s %s %s %s ",\
	 ...
	 if(H->dimensions[0]!=0 && H->dimensions[0] < 3){
		...
		for(i=1;i<H->dimensions[0];i++){
				...
				fprintf(outputfile,"%d,",H->dimensions[i]);
