...
short existing_file(char *name){
	...
	if( (f=fopen(join_strings(name,ascii_ft),"r"))!=NULL ){
		a=1;
		fclose(f);
	}else if( (f=fopen(join_strings(name,ascii_grass),"r"))!=NULL ){
		a=2;
		fclose(f);	
	}else if( (f=fopen(join_strings(name,ascii_esri),"r"))!=NULL ){
		a=3;
		fclose(f);
	}
	return(a);
...
//	a=2 esegue controllo novalues, Mref e UVref input
	...
	if (existing_file(filename)==1){
		filename2=join_strings(filename,ascii_ft);
		if(a>0){
			UV=(T_INIT *)malloc(sizeof(T_INIT));
			if(!UV) t_error("UV was not allocated in read_map");
		}
		f=t_fopen(join_strings(filename,ascii_ft),"r");
		...
		if(existing_file(filename)==2){	
			...
			X0map=header[3];
			Y0map=header[1];
			filename2=join_strings(filename,ascii_grass);
			...
			X0map=header[2];
			Y0map=header[3];
			filename2=join_strings(filename,ascii_esri);
		}else{
