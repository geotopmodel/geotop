...
/** @brief This function calculates the average cloudiness of a dataset of meteo variables recorded in a meteo station where the shortwave radiation is measured.
	 ...
	 * in which the sun rises. The end of the daylight corresponds to the moment (meteo line) when the sun sets.
	 ...
	 */
void cloudiness_MeteoIO(double **meteo, long meteolines, double **horizon, long horizonlines, double lat, double lon,
...
short fill_meteo_data_with_cloudiness_MeteoIO(double **meteo, long meteolines, double **horizon, long horizonlines, double lat,
	...
	if ( (long)meteo[0][iSW] != geotop::input::gDoubleAbsent || ( (long)meteo[0][iSWb] != geotop::input::gDoubleAbsent && (long)meteo[0][iSWd] != geotop::input::gDoubleAbsent ) ){
		...
		cloudiness_MeteoIO(meteo, meteolines, horizon, horizonlines, lat*GTConst::Pi/180., lon*GTConst::Pi/180., ST, Z, sky, SWrefl_surr, cloudtrans, ndivday, rotation, Lozone, alpha, beta, albedo);
...
int main(int argc, char** argv) {
	...
	short success, added_cloud=0,added_wind_xy=0, added_wind_dir=0, added_Tdew=0, added_RH=0, added_Pint=0;
...
//	mio::IOManager iomanager(cfg);
	...
	num_cols = (long)nmet;
	adt->M->horizon=(double***)malloc(adt->M->st->E.size()*sizeof(double**));
	adt->M->horizonlines=(long*)malloc(adt->M->st->E.size()*sizeof(long));
	adt->M->data=(double***)malloc(adt->M->st->E.size()*sizeof(double**));
	adt->M->var=(double**)malloc((adt->M->st->E.size()-1)*sizeof(double*));
	adt->M->numlines=(long*)malloc(adt->M->st->E.size()*sizeof(long));
	success = read_meteostations_file(adt->M->imeteo_stations, adt->M->st, geotop::common::Variables::files[fmetstlist], IT->meteostations_col_names, flog);
	...
	for(size_t i=1; i <= num_met_stat; i++){
	 //	if (met->imeteo_stations->co[1] != geotop::input::gDoubleNoValue) {
		...
		//	read horizon
		adt->M->horizon[i-1] = read_horizon(1, ist, geotop::common::Variables::files[fhormet], IT->horizon_col_names, &num_lines, flog);
		...
		if (geotop::common::Variables::files[fmet] != geotop::input::gStringNoValue){
			...
			adt->M->data[i-1] = read_txt_matrix(temp, 33, 44, IT->met_col_names, nmet, &num_lines, flog);

			if ((long)adt->M->data[i-1][0][iDate12] == geotop::input::gDoubleAbsent && (long)adt->M->data[i-1][0][iJDfrom0] == geotop::input::gDoubleAbsent) {
			...
			short added_JDfrom0 = fixing_dates(ist, adt->M->data[i-1], adt->P->ST, adt->M->st->ST[i], adt->M->numlines[i-1], iDate12, iJDfrom0);

			check_times(ist, adt->M->data[i-1], adt->M->numlines[i-1], iJDfrom0);

			//find clouds
			if(IT->met_col_names[itauC] != geotop::input::gStringNoValue){
				if((long)adt->M->data[i-1][0][itauC] == geotop::input::gDoubleAbsent || adt->P->ric_cloud == 1){
					added_cloud = fill_meteo_data_with_cloudiness(adt->M->data[i-1], adt->M->numlines[i-1], adt->M->horizon[i-1], adt->M->horizonlines[i-1],
							adt->M->st->lat[i], adt->M->st->lon[i], adt->P->ST, adt->M->st->Z[i], adt->M->st->sky[i], 0.0, adt->P->ndivdaycloud, adt->P->dem_rotation,
							adt->P->Lozone, adt->P->alpha_iqbal, adt->P->beta_iqbal, 0.);
				}
			}

			rewrite_meteo_files(adt->M->data[i-1], adt->M->numlines[i-1], IT->met_col_names, temp.c_str(), added_JDfrom0, added_wind_xy, added_wind_dir, added_cloud, added_Tdew, added_RH, added_Pint);
			//IT->met_col_name;
		}
	}
	fclose(flog);
