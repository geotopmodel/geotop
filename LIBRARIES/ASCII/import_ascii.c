...
#include "turtle.h"
#include "t_utilities.h" /* line addded by Emanuele Cordano on 1 september 2009 */
...

{
	...
	short sgn, end=0;
	double *dtm;
	char *grass=join_strings(name,ascii_grass);
...
//				printf("Error: - not followed by a number for r=%ld c=%ld, file %s",r,c,grass); /* commeted by Emanuele Cordano on 24/9/9 */
				t_error("Fatal error");
			}
		}
		if(ch[0]>=48 && ch[0]<=57){
			do{
				cont+=1;
				if(cont>=max_figures){
					printf("Increase max_figures_in_a_number in file %s",grass);
					t_error("Fatal error");
				}
				ch[cont]=fgetc(f);
	...
	if(header[1]>=header[0]){
		printf("In file %s south larger than or equal to north",grass);
		t_error("Fatal error");
	}
	if(header[3]>=header[2]){
		printf("In file %s west larger than or equal to east",grass);
		t_error("Fatal error");
	}
	if(header[4]<=0 || header[5]<=0){
		printf("In file %s nrows or ncols negative or null",grass);
		t_error("Fatal error");
	}
	...
	for(r=1;r<=nr;r++){
		...
		do{
			...
			do{
				...
				if(ch[0]==10){	//end of line
					if(c==nc+1 || r>nr){
						...
						printf("Number of cols less than declared in row %ld in file %s",r,grass);
						t_error("Fatal error");
					}
				}
				if(ch[0]==-1){	//end of file
					if( ((r==nr)&&(c==nc+1)) || (r>nr) ){
						...
						printf("Number of rows less than declared in file %s",grass);
						t_error("Fatal error");
					}
				}
				if(ch[0]==58){
					printf("Header cannot consist of more than 6 lines, error in file %s",grass);
					t_error("Fatal error");
				}
			...
			if(end==0){
				...
				if(ch[0]==43 || ch[0]==45){
					ch[0]=fgetc(f);
					if(ch[0]<=45 || ch[0]==47 || ch[0]>=58){
						printf("Error: +- not followed by a number for r=%ld c=%ld, file %s",r,c,grass);
						t_error("Fatal error");
					}
				}
				if(ch[0]>=48 && ch[0]<=57){
		...
		if(c>nc && r<=nr){
			printf("Number of cols higher than declared in row %ld in file %s",r,grass);
			t_error("Fatal error");
		}
		if(ch[0]==-1 && r<nr){
			printf("Number of rows less than declared in file %s",grass);
...

{
	...
	for(i=0;i<=4;i++){
		...
		if(ch[0]==45){
			...
			ch[0]=fgetc(f);
			if(ch[0]<=45 || ch[0]==47 || ch[0]>=58){
			//	printf("Error: +- not followed by a number for r=%ld c=%ld, file %s",r,c,join_strings(name,ascii_esri)); /* modified by Emanuele Cordano on 24 Sept 2009 */
				t_error("Fatal error");
			}
		}
		if(ch[0]>=48 && ch[0]<=57){
	...
	if(header[0]<=0 || header[1]<=0){
		printf("In file %s nrows or ncols negative or null %lf %lf",join_strings(name,ascii_esri),header[0], header[1]);
		t_error("Fatal error");
	}
