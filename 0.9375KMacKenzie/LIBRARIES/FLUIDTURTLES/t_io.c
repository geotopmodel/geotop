...
long i,j,tmp=0,chksum=0; //count=0,
...

{
    ...
    char buffer[64*FILENAME_MAX],*bf=NULL,*pathfile="$WorkingPath";
    long len,i;
	short a;
    FILE *istream;
    ...
    if(istream){
		...
		scanf("%s",buffer); /* This line is note clear, comment by Emanuele Cordano on 1 September 2009 */
		len=64*FILENAME_MAX;
...
char *pathfile="\0",*S=".inpts"; //*fullname="\0",
FILE *istream;
...

{
    ...
    char *name="\0",*pathfile="\0",*S=".inpts";//*fullname="\0"
    FILE *istream;
    static STRINGBIN *s;
    long position=-1;
    short sign=0;
...
STRINGBIN *read_filenames(char *working_directory,char *program, char *extension, char *position){
	...
	STRINGBIN *strg=NULL;/* abbed by Emanuele Cordano on 24/9/09 */
	FILE *istream;
	long i,pos=-1;
...
DOUBLEVECTOR *read_parameters(char *working_directory,char *program, char *extension, char *position){
	...
	DOUBLEVECTOR *v=NULL; /* added by Emanuele Cordano on 24/9/09 */
	FILE *istream;
	long pos=-1;
	char *temp1,*temp0; /* added by Emanuele Cordano on 24/9/09 */

	temp0=join_strings(program,extension);
	temp1=join_strings(working_directory,temp0);
	istream=fopen(temp1,"r");
...
long buffer_index=0,pos=0;//blocksnumber=0
long blocks;
...
long buffer_size=0,i, jj; //buffer_index=0,blocksnumber=0,
...
char curl,ch=EOF; /* added by Emanuele Cordano on Sept 2009 */

const char ocurl='{', ccurl='}';

//short ind=1;

long buffer_size=0,i=0, jj; //,blocksnumber=0,buffer_index=0
...
long buffer_size=0,i=0, jj=0; //buffer_index=0,blocksnumber=0
...
free_header(h);/* modified by Emanuele Cordano on 8 september 2009 */
...
const char ocurl='{', ccurl='}';
//short ind=1;
long buffer_size=0,i,jj;//blocksnumber=0,buffer_index=0
short *numbers;
...
long strings_size=0,char_counter=0,strings_counter=0; //blocksnumber=0,strings_index=0,

long numbers_size=0;//number_index=0;
...
//free(h.name);/* modified by Matteo on 31 Augyst 2009 */
//free(&h);
free_header(h);

free(vec->element); /* modified by Emanuele Cordano on 31 Augyst 2009 */
free(vec);
free(strings);
...
char curl,ch=EOF; /* added by Emanuele Cordano */
const char ocurl='{', ccurl='}';
HEADER h;
long i;
...
char curl,ch=EOF; /* added by Emanuele Cordano on 24/9/9 */
const char ocurl='{', ccurl='}';
HEADER h;
long i;
...
char curl,ch=EOF; /* ch=EOF added by Emanuele Cordano on 24/9/9 */
const char ocurl='{', ccurl='}';
HEADER h;
long i;
...
char ch;
HEADER h;
CHARVECTOR *C=NULL; /* abbed by Emanuele Cordano on 24/9/09 */
long u;
...
char ch;
HEADER h;
LONGVECTOR *C=NULL; /* added by Emanuele Cordano */
long u;
...
char ch;
HEADER h;
DOUBLEMATRIX *C=NULL;  /* added by Emanuele Cordano on Sept 2009 */
char *temp;
long u;
...
if(inputfile==NULL){
	...
	if(query_for_token(inputfile,"->")) {
	   	...
	   	if (OPENYES==1){
	  		...
	  		OPENYES=1;
	  		temp=get_phrase(inputfile,';');
	  		strcpy(EXTERNAL_FILE_NAME,temp);
	  		free(temp);
...
char ch;
LONGVECTOR *P;
DOUBLEBIN *C=NULL; /* abbed by Emanuele Cordano on 24/9/09 */
long u;
