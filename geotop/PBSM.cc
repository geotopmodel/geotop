...
void Pbsm (long r, long c, double Fetch, double N, double dv, double Hv, double rho_sn, double zmeas, double V, double Ta, double RH, 
	...
	if(Fetch>=300){
		...
		i=0;
		Z0 = CC2 * 0.07519 * pow(Ustar, 2.) / (2.*GTConst::GRAVITY) + Zstb;//Liston 1998
		F = (Ustar/GTConst::ka) * log(zmeas/Z0) - V;
		do{
			Ustar1 = Ustar;
			F1 = (1./GTConst::ka) * log(zmeas/Z0) - (Ustar/GTConst::ka) * (CC2*0.07519/GTConst::GRAVITY) * Ustar / Z0;
			k = 0;
			do{
		...
		if(Ustar > Usthr){
			...
			if(Nsalt > 0){
				...
				SigmaZ = Sigma * (1.019 + 0.027 * log(Hsalt)); //{ Eq. 6.20, Revised in May. 1997}
				...
				Mpm = 4.0/3.0 * GTConst::Pi * GTConst::rho_i * Mpr*pow(Mpr,2.0) *(1.0 + 3.0/Alpha + 2.0/pow(Alpha,2.0));
...
double sublimation(double Z){	
	...
	C = 1.0/(Diff * SvDens * Nuss);
	DmDt = (2.0*GTConst::Pi * Mpr * SigmaZ)/(GTConst::Ls*B/A + C);
	Mpm = 1.333 * GTConst::Pi * GTConst::rho_i * pow(Mpr,3.0) * (1.0 + 3.0/Alpha + 2.0/pow(Alpha,2.0));  //{Eq. 6.16} {Gamma Dist. Corr.}
