...
//	if(sl->SS->T->co[n][i]<thresh){
	if(sl->SS->T[n][i]<thresh){
		...
		if(out==1){
		//	table += (sl->SS->thi->co[n-1][i]/(sl->SS->thi->co[n-1][i]+sl->th->co[n-1][i]-sl->pa[ty][jres][n-1]))*sl->pa[ty][jdz][n-1];
			table += (sl->SS->thi[n-1][i]/(sl->SS->thi[n-1][i]+sl->th[n-1][i]-sl->pa[ty][jres][n-1]))*sl->pa[ty][jdz][n-1];
		}else {
			table = 0.;
		}
...
//	if(sl->Ptot->co[n][i]<thresh){
	if(sl->Ptot[n][i]<thresh){
		...
		//	table += ( 0.5*sl->pa[ty][jdz][n+1] - sl->Ptot->co[n+1][i]*0.5*(sl->pa[ty][jdz][n]+sl->pa[ty][jdz][n+1])/(sl->Ptot->co[n+1][i]-sl->Ptot->co[n][i]) );
			table += ( 0.5*sl->pa[ty][jdz][n+1] - sl->Ptot[n+1][i]*0.5*(sl->pa[ty][jdz][n]+sl->pa[ty][jdz][n+1])/(sl->Ptot[n+1][i]-sl->Ptot[n][i]) );
		}
	}
...
//	if(sl->Ptot->co[n][i]<thresh){
	if(sl->Ptot[n][i]<thresh){
		...
		if (out==1){
		//	table += ( 0.5*sl->pa[ty][jdz][n] - sl->Ptot->co[n][i]*0.5*(sl->pa[ty][jdz][n-1]+sl->pa[ty][jdz][n])/(sl->Ptot->co[n][i]-sl->Ptot->co[n-1][i]) );
			table += ( 0.5*sl->pa[ty][jdz][n] - sl->Ptot[n][i]*0.5*(sl->pa[ty][jdz][n-1]+sl->pa[ty][jdz][n])/(sl->Ptot[n][i]-sl->Ptot[n-1][i]) );
		}else {
