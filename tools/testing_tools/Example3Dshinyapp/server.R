...
wpath_3D <- '../../../examples/3D' 
wpaths <- list.files(wpath_3D,recursive=FALSE,full.name=TRUE)
names(wpaths) <- list.files(wpath_3D,recursive=FALSE,full.name=FALSE)

wpaths <- wpaths[names(wpaths)!="PSQL_test"]
...
for (it in names(keywords.out)) {
	...
	keywords.out[[it]]$level <- paste(1:20,collapse=",")
