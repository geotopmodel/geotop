...
static std::vector<std::string> ReadHeader(FILE *f,
                                           std::string filename,
                                           long *num_cols);
...
static double *readline_of_numbers(FILE *f,
                                   long comment_char,
                                   long sep_char,
                                   long *components,
                                   short *endoffile,
...
// static long* ColumnCoder(std::string filename, std::vector<std::string>
// ColDescr,
//                         long max_num_cols, std::vector<std::string> header,
//                         long num_cols_header, FILE *flog);

static double **read_datamatrix(FILE *f,
                                long comment_char,
                                long sep_char,
                                long number_lines,
                                long components_header);
                      ...
                      short *endoffile)
{
  ...
  *endoffile = 0;
  ...
  while (
    *endoffile == 0 &&
    (c[0] <= 42 || (c[0] >= 58 && c[0] <= 64) || c[0] == 96 || c[0] >= 123) &&
    c[0] != comment_char);
...
static long *ColumnCoder(std::string filename,
      ...
      for (j = 0; j < num_cols_header; j++)
        {
          ...
          if (lowercaseColDescr == header[j] && coder[i] == -1 &&
              geotop::input::gStringNoValue != header[j])
            {
              coder[i] = j;
              lg->logf("Column %ld in file %s assigned to %s", j + 1,
                       filename.c_str(), ColDescr[i].c_str());
            }
...
long count_lines(std::string meteo_file_name,
                 long comment_char,
                 long sep_char)
{
  FILE *f;
  ...
  do
    {
      line = readline_of_numbers(f, comment_char, sep_char, &components,
                                 &endoffile, &success);
      if (success == 1)
        {
          free(line);
...
double **read_txt_matrix(std::string filename,
                         long comment_char,
                         long sep_char,
                         std::vector<std::string> Col_Descr,
                         long ncolsCol_Descr,
                         long *nlines)
{
  /*Read header, and create a **double with the same columns as the header. Then
   fill with geotop::input::gDoubleAbsent the columns missing with respect to
   Col_Descr*/
...
double **read_txt_matrix_2(std::string filename,
                           long comment_char,
                           long sep_char,
                           long ncolsCol_Descr,
                           long *nlines)
{
  /*Read header, and create a **double with the same columns as Col_Descr,
   * filling with geotop::input::gDoubleNoValue the missing columns*/
