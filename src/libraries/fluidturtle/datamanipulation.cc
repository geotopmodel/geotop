...

{
  ...
  for (i=1; i<=cleandata->nrh-1; i++)
    {
      W->co[i][1]=(cleandata->co[i+1][2]-cleandata->co[i][2])/
                  (cleandata->co[i+1][1]-cleandata->co[i][1]);
...
void interpolate_floatmatrix(FLOATMATRIX *matrice, float dt, float istante,
                             FLOATVECTOR *vettore)
...
*/
{
  ...
  for (j=1; j<=matrice->nch; j++)
    {
      vettore->co[j]=matrice->co[i1][j]+(matrice->co[i1+1][j]-matrice->co[i1][j])
                     *resto;
    }
}
...
DOUBLEMATRIX *ricampiona(DOUBLEMATRIX *cleandata, float ti2, float dt2,
                         long n2, float dt1)
...
DOUBLEMATRIX *cleandata_matrix(DOUBLEMATRIX *data, FLOATVECTOR *V,
                               SHORTMATRIX *control)
...
void fill_data(DOUBLEMATRIX *cleandata,SHORTMATRIX *control,long j,long n_i,
               long n_f,double x_i,double x_f)
