...

{
  ...
  if (output==NULL)
  ...
  else
    {
      for (i=m->nrl; i<=m->nrh; i++)
        {
          tmp=fwrite((double *)&(m->co[i][m->nrl]),sizeof(double),m->nch-m->ncl+1,
                     output);
          if (tmp!=EOF)
            {
              count+=tmp;
                          ...
                          char *extension, char *position)
{
  ...
  istream=fopen(join_strings(working_directory,join_strings(program,extension)),
                "r");
...
int header_scan(FILE *inputfile,HEADER *h)
{
  ...
  if (j > len)
    {
      h->category=
        3;/* nel caso la keyword NON sia tra quelle ammesse, viene posta per default a 3 */
    }
  else
    {
      h->category=j;
...
void justread_floatmatrix(FILE *inputfile, FLOATMATRIX *C, char *mode,
                          short print)
      ...
      if (query_for_token(inputfile,"->"))
        {
          if (OPENYES==1)
          ...
          else
            {
              ...
              else if (!strcmp(EXTERNAL_FILE_NAME,OLD_NAME)
                       && EXTERNAL_FILE_POSITION!=SEEK_SET)
                {
                  EXTERNAL_FILE=t_fopen(EXTERNAL_FILE_NAME,"r");
  ...
  if (inputfile==NULL)
  ...
  else
    {
      ...
      if (curl!=ocurl)
        ...
        {
          h->dimensions[0]=3;
          fscanf(inputfile," %ld ,  %ld , %ld",&(h->dimensions[1]),&(h->dimensions[2]),
                 &(h->dimensions[3]));
        }
