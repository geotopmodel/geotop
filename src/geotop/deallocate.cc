                 ...
                 Times *times)
{
  ...
  if (par->blowing_snow == 1) { deallocate_statevar_1D(snow->S_for_BS); }

  if (par->blowing_snow == 1) free(snow);
  ...
  if (par->state_pixel == 1)
    {
      ...
      if (geotop::common::Variables::files[fliqzav] !=
          ...
          geotop::input::gStringNoValue)
        {
          sl->thzavplot.resize(sl->thzavplot.getRows(), sl->thzavplot.getCols(),
                               0.0);
        }
      if (geotop::common::Variables::files[ficezav] !=
          ...
          geotop::input::gStringNoValue)
        {
          sl->thizavplot.resize(sl->thizavplot.getRows(), sl->thizavplot.getCols(),
                                0.0);
        }
  ...
  if (par->output_snow_bin == 1)
    {
      if (geotop::common::Variables::files[fsnowmelt] !=
          geotop::input::gStringNoValue)
        snow->MELTED.resize(snow->MELTED.size(), 0.);
      ...
      if (geotop::common::Variables::files[fswe] !=
          ...
          par->blowing_snow == 1)
        {
          initmatrix(0.0, snow->Wtrans_plot, land->LC,
  ...
  if (par->output_surfenergy_bin == 1)
    {
      ...
      if (geotop::common::Variables::files[fG] != geotop::input::gStringNoValue)
        egy->SEB_mean.resize(egy->SEB_mean.size(), 0.);
