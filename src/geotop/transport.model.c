/*****************************************************************************************************
Transport model by Stefano Endrizzi and Florian Marshall 2013
*****************************************************************************************************/
...
//main part, which check's the timestep, and if it's valid the transport is calculated
short transport_model(double Dt, ALLDATA *adt){

	if ( Dt > Dt_transport_model(0.8, adt->W->M, adt->W->C, adt->W->H, adt->W->K, adt->W->D, adt)){
		return 1;
	}else{
		calculate_new_concentrations(Dt, adt->W->M, adt->W->C, adt->W->H, adt->W->K, adt->W->D, adt);
		return 0;
	}
...
//timestep for transport is calculated and returned
double Dt_transport_model(double Courant, DOUBLEVECTOR *M, DOUBLEVECTOR *C, DOUBLEVECTOR *H, DOUBLEVECTOR *K, DOUBLEVECTOR *D, ALLDATA *adt){
	...
	double Dt=1.E99;//initial timestep
				...
				if (j > i) {
					...
					if ( (i>n && d==1) || (i<=n && d<=5) || (i<=n && d==6 && l>0 && ch>0) ) {
						...
						if(l>0){
							...
							diff = 1.e-10;//D->co[cnt] * (C->co[j] - C->co[i]);
