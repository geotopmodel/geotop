...
//  logfile =  nullptr;  //join_strings(WORKING_DIRECTORY, logfile_name);
      ...
      if (existing_file_wext(SuccessfulRunFile,"") == 1)
        {
          ...
          f = fopen(FailedRunFile, "w");
          fprintf(f,
                  "This simulation has successfully reached the end, you cannot recover it.\n");
          fclose(f);
          t_error("Fatal Error! Geotop is closed. See failing report.");
        }
      ...
      if (strcmp(files[fmet], string_novalue) != 0)
        {
              ...
              if ((long)met->data[i-1][0][itauC] == number_absent || par->ric_cloud == 1)
                {
                  added_cloud = fill_meteo_data_with_cloudiness(met->data[i-1],
                                                                met->numlines[i-1], met->horizon[i-1], met->horizonlines[i-1],
                                                                met->st->lat->co[i], met->st->lon->co[i], par->ST, met->st->Z->co[i],
                                                                met->st->sky->co[i], 0.0, par->ndivdaycloud, par->dem_rotation,
          ...
          if (par->wind_as_xy == 1)
            {
              added_wind_xy = fill_wind_xy(met->data[i-1], met->numlines[i-1], iWs, iWdir,
                                           iWsx, iWsy, IT->met_col_names[iWsx], IT->met_col_names[iWsy]);
            }
          ...
          if (par->wind_as_dir == 1)
            {
              added_wind_dir = fill_wind_dir(met->data[i-1], met->numlines[i-1], iWs, iWdir,
                                             iWsx, iWsy, IT->met_col_names[iWs], IT->met_col_names[iWdir]);
            }
          ...
          if ( par->linear_interpolation_meteo->co[i] == 1
               && (long)met->data[i-1][0][iPrec] != number_absent)
            {
              ...
              fprintf(f,
                      ...
                      i);
              fprintf(f,
                      "If you want to use precipitation as volume, you cannot set keyword LinearInterpolation at 1.\n");
              fclose(f);
          ...
          rewrite_meteo_files(met->data[i-1], met->numlines[i-1], IT->met_col_names,
                              temp, added_JDfrom0, added_wind_xy, added_wind_dir, added_cloud, added_Tdew,
                              added_RH, added_Pint);
  ...
  do
    {
      ...
      if ( (long)met->data[met->nstsrad-1][0][iSW]!=number_absent
           || ((long)met->data[met->nstsrad-1][0][iSWb]!=number_absent
               && (long)met->data[met->nstsrad-1][0][iSWd]!=number_absent ) ) a=1;
    }
  ...
  do
    {
      ...
      if ( (long)met->data[met->nstcloud-1][0][iC]!=number_absent
           || (long)met->data[met->nstcloud-1][0][itauC]!=number_absent ) a=1;
    }
  ...
  initialize_shortmatrix(land->shadow,
                         0);/* initialized as if it was always NOT in shadow*/
      ...
      for (l=1; l<=met->st->Z->nh; l++)
        {
          if (0.001*land->ty->co[i][jHveg]>met->st->Vheight->co[l]
              || 0.001*land->ty->co[i][jHveg]>met->st->Theight->co[l])
            {
              f = fopen(FailedRunFile, "w");
              fprintf(f,
                      "hc:%f m, zmu:%f m, zmt:%f m - hc must be lower than measurement height - land cover %ld, meteo station %ld\n",
                      0.001*land->ty->co[i][jHveg],met->st->Vheight->co[l],met->st->Theight->co[l],
                      i,l);
              fclose(f);
  ...
  if (strcmp(files[ftsteps], string_novalue) != 0)
    {
      ...
      times->Dt_matrix = read_txt_matrix_2(temp, 33, 44, max_cols_time_steps_file+1,
                                           &num_lines);
      free(temp);
          ...
          for (l=1; l<=Nl; l++)
            {
              ...
              cnet->th->co[l][i] = teta_psi(Fmin(cnet->SS->P->co[l][i],
                                                 psi_saturation(cnet->SS->thi->co[l][i], sl->pa->co[sy][jsat][l],
                                                                sl->pa->co[sy][jres][l], sl->pa->co[sy][ja][l], sl->pa->co[sy][jns][l],
                                                                1.-1./sl->pa->co[sy][jns][l])),
                                            cnet->SS->thi->co[l][i], sl->pa->co[sy][jsat][l], sl->pa->co[sy][jres][l],
                                            sl->pa->co[sy][ja][l],
                                            sl->pa->co[sy][jns][l], 1.-1./sl->pa->co[sy][jns][l], PsiMin,
                                            sl->pa->co[sy][jss][l]);
            }
  ...
  if (par->Zboundary < 0)
    {
      f = fopen(FailedRunFile, "w");
      fprintf(f,
              "Z at which 0 annual temperature takes place is not lower than the soil column\n");
      fclose(f);
              ...
              if (D<0 || SWE<0)
                {
                  f = fopen(FailedRunFile, "w");
                  fprintf(f,
                          "Error: negative initial snow depth %e or snow water equivalent %e\n",D,SWE);
                  fclose(f);
              ...
              else if (D<1.E-5 && SWE>1.E-5)
                {
                  f = fopen(FailedRunFile, "w");
                  fprintf(f,
                          "Error: Initial snow water equivalent %e > 0 and initial snow depth %e\n",SWE,
                          D);
                  fclose(f);
              ...
              else if (D>1.E-5 && SWE<1.E-5)
                {
                  f = fopen(FailedRunFile, "w");
                  fprintf(f,
                          "Error: Initial snow depth %e > 0 and initial snow water equivalent %e\n",D,
                          SWE);
                  fclose(f);
  ...
  if (recovered > 0)
    {
      ...
      assign_recovered_map_long(old, par->recover, files[rns], snow->S->lnum, par,
                                land->LC);
  ...
  if (par->max_glac_layers>0)
    {
      ...
      if (recovered > 0)
        {
          assign_recovered_map_long(old, par->recover, files[rni], glac->G->lnum, par,
                                    land->LC);
          assign_recovered_tensor(old, par->recover, files[rDzi], glac->G->Dzl, par,
                                  land->LC);
          assign_recovered_tensor(old, par->recover, files[rwli], glac->G->w_liq, par,
                                  land->LC);
          assign_recovered_tensor(old, par->recover, files[rwii], glac->G->w_ice, par,
                                  land->LC);
          assign_recovered_tensor(old, par->recover, files[rTi], glac->G->T, par,
                                  land->LC);
        }
    }
  ...
  write_output_headers(met->st->Z->nh, times, wat, par, top, land, sl, egy,
                       snow, glac);
                    ...
                    FILE *flog)
{
  ...
  if (flag == 1)
  ...
  else
    {
      ...
      f = fopen(FailedRunFile, "w");
      fprintf(f,
              "Error: It is impossible to proceed without giving the digital elevation model\n");
      fclose(f);
              ...
              if ((long)land->LC->co[r][c] != number_novalue)
                {
                  if ((long)land->LC->co[r][c] < 1
                      || (long)land->LC->co[r][c] > par->n_landuses)
                    {
                      f = fopen(FailedRunFile, "w");
                      fprintf(f,
                              "Error: It is not possible to assign Value < 1 or > n_landuses to the land cover type\n");
                      fclose(f);
      ...
      for (i=1; i<=par->chkpt->nrh; i++)
        {
          ...
          par->rc->co[i][1]=row(par->chkpt->co[i][ptY], top->Z0->nrh, UV,
                                number_novalue);
          par->rc->co[i][2]=col(par->chkpt->co[i][ptX], top->Z0->nch, UV,
                                number_novalue);
  ...
  if (flag == 1)
                  ...
                  if (sl->type->co[r][c] < 1 || sl->type->co[r][c] > par->nsoiltypes)
                    {
                      f = fopen(FailedRunFile, "w");
                      fprintf(f,
                              "Error: It is not possible to assign Value < 1 or > nsoiltypes to the soil type map");
                      fclose(f);
    ...
    {
      M=copydoublematrix_const(par->soil_type_land_default, land->LC,
                               (double)number_novalue);
      sl->type=copylong_doublematrix(M);
    }
  ...
  if (flag == 1)
    {
      top->slope=read_map(2, files[fslp], land->LC, UV,
                          (double)number_novalue);    //reads in degrees
    }
  else
    {
      top->slope=find_max_slope(top->Z0, top->dzdE, top->dzdN,
                                (double)number_novalue);
    }
  ...
  if (flag == 1)
    ...
    {
      top->aspect=find_aspect(top->Z0, top->dzdE, top->dzdN,
                              (double)number_novalue);
    }
              ...
              if ((long)land->LC->co[r][c]!=number_novalue)
                {
                  if (top->pixel_type->co[r][c]!=0 && top->pixel_type->co[r][c]!=1
                      ...
                      && top->pixel_type->co[r][c]!=-1)
                    {
                      f = fopen(FailedRunFile, "w");
                      fprintf(f,
                              "Error: Only the following values are admitted in the network map: -1, 0, 1, 2, 10\n");
                      fclose(f);
  ...
  if (read_curv==1)
    {
      if (read_dem==0)
      ...
      else
        {
          ...
          multipass_topofilter(par->lowpass_curvatures, Z, Q, (double)number_novalue,
                               1);
