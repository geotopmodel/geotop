...
void Tcanopy(long r,
             ...
             long sy,
             double *T,
             GeoVector<double> &soil_evap_layer)
{
  double C, C0;
  double A = 1.0;
  ...
  C0 = land(lu, jcd) * LSAI * GTConst::c_can + GTConst::c_ice * Wcsn +
       GTConst::c_liq * Wcrn;
  C = C0;
  ...
  if (A < 1)
    {
      ...
      *Qs = 0.5 * Qg0 + 0.5 * Qa;
      canopy_fluxes(r, c, T00, Tg0, Ta, Qg0, Qa, zmu, zmT, z0, z0s, d0, z0r, hveg,
                    v, LR, P, SW, LW, e, LSAI, decaycoeff0, land, lu, Wcrn0,
                    Wcrnmax, Wcsn0, Wcsnmax, &subl_can, Etrans, LWv, LWg, Hv, LEv,
                    &h0, &dhdT, Ts, Qs, Qv, ruc, froot, theta, soil_transp_layer,
                    chgsgn, Lobukhov, par, n, rm, rh, rv, rc, rb, u_top, decay,
                    Locc, LWup_above_v, psi, soil, sy, &alpha, &beta, T,
                    soil_evap_layer);
    }
  ...
  *Qs = 0.5 * Qg + 0.5 * Qa;
  canopy_fluxes(r, c, T11, Tg, Ta, Qg, Qa, zmu, zmT, z0, z0s, d0, z0r, hveg, v,
                LR, P, SW, LW, e, LSAI, decaycoeff0, land, lu, Wcrn0, Wcrnmax,
                Wcsn0, Wcsnmax, &subl_can, Etrans, LWv, LWg, Hv, LEv, &h1,
                &dhdT, Ts, Qs, Qv, ruc, froot, theta, soil_transp_layer, chgsgn,
                Lobukhov, par, n, rm, rh, rv, rc, rb, u_top, decay, Locc,
  ...
  do
    {
      ...
      // eq. C*(T11-T10)/Dt + C*(T10-T00)/Dt = cost + h(T10) + dh(T10)/dT *
      // (T11-T10)  eq. T11-T10 = ( cost + h(T10) - C*(T10-T00)/Dt ) / ( C/Dt -
      // dh/dT )
      ...
      if (DT != DT)
        {
          ...
          fprintf(f,
                  "Error:: NwRph Tcanopy T00:%f T10:%f SWv:%f h0:%f h1:%f dhdT:%f "
                  "C:%f Wcsn:%f Wcrn:%f %ld %ld\n",
      ...
      do
        {
          ...
          if (T11 > 0 && Wcsn > 0)    // melting
            {
              melt_can =
                Fmin(Wcsn, GTConst::c_ice * Wcsn * (T11 - 0.0) / GTConst::Lf);
            ...
            {
              melt_can =
                -Fmin(Wcrn, GTConst::c_liq * Wcrn * (0.0 - T11) / GTConst::Lf);
              T11p = T11 - GTConst::Lf * melt_can / C;
              Wcsn -= melt_can;
              Wcrn += melt_can;
          ...
          C = land(lu, jcd) * LSAI * GTConst::c_can + GTConst::c_ice * Wcsn +
              GTConst::c_liq * Wcrn;
          C = (C + C0) / 2.;

          canopy_fluxes(r, c, T11p, Tg, Ta, Qg, Qa, zmu, zmT, z0, z0s, d0, z0r,
                        hveg, v, LR, P, SW, LW, e, LSAI, decaycoeff0, land, lu,
                        Wcrn, Wcrnmax, Wcsn, Wcsnmax, &subl_can, Etrans, LWv, LWg,
                        Hv, LEv, &h1, &dhdT, Ts, Qs, Qv, ruc, froot, theta,
                        soil_transp_layer, chgsgn, Lobukhov, par, n, rm, rh, rv, rc,
                        rb, u_top, decay, Locc, LWup_above_v, psi, soil, sy, &alpha,
                        &beta, T, soil_evap_layer);
  ...
  *dWcsn = Wcsn - Wcsn0;
  ...
  Turbulence::turbulent_fluxes(*ruc, *ruc / beta, P, *Ts, Tg, *Qs, alpha * Qg,
                               alpha * dQgdT, *Hg, *dHgdT, *Eg, *dEgdT);
                   ...
                   GeoVector<double> &soil_evap_layer)
{
  ...
  do
    {
      ...
      // neglects stability corrections, if the number of iterations in Tcanopy is
      // larger than a threshold
      if (chgsgn > max_chgsgn) MO = 4;

      // apply Monin-Obukhov from canopy air to atmosphere
      Turbulence::aero_resistance(zmu, zmT, z0, d0, z0r, v, Ta, Ts0, Qa, *Qs, P,
                                  LR, Lobukhov, rm, rh, rv, par->state_turb, MO,
                                  par->maxiter_Businger);

      // friction speed
      u_star = sqrt(v / (*rm));

      // wind speed at the top of the canopy
      *u_top = (u_star / GTConst::ka) *
               Turbulence::CZ(MO, hveg, z0, d0, *Lobukhov, (&Turbulence::Psim));
      ...
      do
        {
          ...
          veg_transmittance(par->stabcorr_incanopy, v, u_star, *u_top, hveg, z0s,
                            z0, d0, LSAI, decaycoeff0, *Lobukhov, Loc, rb, ruc,
                            decay);

          Turbulence::find_actual_evaporation_parameters(
            r, c, alpha, beta, soil_evap_layer, theta, soil, sy, T, psi, P, *ruc,
            Ta, Qa, Qgsat, n);
          ...
          if ((*Qv) < (*Qs))    // condensation
            ...
            {
              canopy_evapotranspiration(*rb, Tv, Qa, P, SW, theta, land, lu, soil, sy,
                                        froot, &ft, soil_transp_layer);
              R = fw + (1.0 - fw) * ft;
            }
          *rc = (*rb) / R;
          ...
          *Ts = (Ta / (*rh) + Tg / (*ruc) + Tv / (*rb)) /
                (1. / (*rh) + 1. / (*ruc) + 1. / (*rb));
          *Qs = (Qa / (*rv) + (*alpha) * Qgsat * (*beta) / (*ruc) + (*Qv) / (*rc)) /
                (1. / (*rv) + (*beta) / (*ruc) + 1. / (*rc));

          Hg = air_cp((*Ts + Tg) / 2.) *
               air_density((*Ts + Tg) / 2., (*Qs + (*alpha) * Qgsat) / 2., P) *
               (Tg - (*Ts)) / (*ruc);

          //      -u*^3
          // Loc = ------------------    Below Canopy Monin-Obukhov length
          // (Niu&Yang)
          //       k(g/T)(Hg/(rho*C))

          Loc = -pow(u_star, 3.0) /
                (GTConst::ka * (GTConst::GRAVITY / (*Ts + GTConst::tk)) *
                 (Hg / (air_density(*Ts, *Qs, P) * air_cp(*Ts))));
  ...
  // CANOPY FLUXES
  Turbulence::turbulent_fluxes(*rb, *rc, P, *Ts, Tv, *Qs, *Qv, dQvdT, *H, dHdT,
                               E, dEdT);

  // Et from transpiration, E-Et condensation or evaporation/sublimation from
  // water on the canopy
  ...
  if (E > 0)    // evaporation or sublimation
    {
      *Esubl = E * fw / R;
      ...
      if (fwliq + fwice > 0)
        {
          Lv = Lt + GTConst::Lf * fwice / (fwliq + fwice);  // linear interpolation
          // to decide if
          // sublimation or
          // condensation occurs
        }
  ...
  if (*h != (*h))
    {
      f = fopen(geotop::common::Variables::FailedRunFile.c_str(), "w");
      fprintf(f,
              "Error:: No value in canopy fluxes Loc:%e v:%f rm:%e Ts:%f Tv:%f "
              "Ta:%f Tg:%f Hg:%f %ld %ld\n",
...
void shortwave_vegetation(double Sd,
                          double Sb,
                          double x,
                          double fwsn,
                          double wsn,
                          ...
                          double *Sup_above)
{
  ...
  double b, c, d, f, h, s, u1, u2, u3, s1, s2, p1, p2, p3, p4, d1, d2, h1, h2,
         h3, h4, h5, h6, h7, h8, h9, h10, Iupb, Iupd, Idwb, Idwd, Ib, Id;
  ...
  wBd = (0.5 * (R + T + pow(0.5 * (1.0 + C), 2.0) * (R - T))) * (1.0 - fwsn) +
        wsn * Bsnd * fwsn;
  wBb = (((1.0 + xm * K) / (xm * K)) * 0.5 * w *
         ((G / (x * phi2 + G)) *
          (1.0 - (x * phi1 / (x * phi2 + G)) *
           log((x * phi1 + x * phi2 + G) / (x * phi1))))) *
        (1.0 - fwsn) +
        wsn * Bsnb * fwsn;
  ...
  h2 = ((d - h1 * p3 / s) * (u1 - xm * h) / s1 -
        p2 * (d - c - h1 * (u1 + xm * K) / s) * s2) /
       d1;
  h3 = -((d - h1 * p3 / s) * (u1 + xm * h) * s1 -
         p1 * (d - c - h1 * (u1 + xm * K) / s) * s2) /
       d1;
  h4 = -f * p3 - c * d;
  h5 =
    -(h4 * (u2 + xm * h) / (s * s1) + (u3 - h4 * (u2 - xm * K) / s) * s2) / d2;
  ...
  if (x > 0)
    {
      *Sv = Sb * Ib + Sd * Id;
      *Sg =
        Sb * (1.0 - Agb) * exp(-K * L) + (Sb * Idwb + Sd * Idwd) * (1.0 - Agd);
...
void longwave_vegetation(double Lin,
                         double eg,
                         double Tg,
                         double Tv,
                         double L,
                         ...
                         double *Lup_above)
{
  ...
  *Lv = ev * (1.0 + (1.0 - eg) * (1.0 - ev)) * Lin + ev * eg * SB(Tg) -
        (2.0 - ev * (1.0 - eg)) * ev * SB(Tv);
  *Lg = eg * Lvdw - eg * SB(Tg);
  *dLv_dTv = -(2.0 - ev * (1.0 - eg)) * ev * dSB_dT(Tv);

  *Lup_above = (1.0 - ev) * (1.0 - eg) * (1.0 - ev) * Lin +
               (1.0 - ev) * eg * SB(Tg) +
               (1.0 + (1.0 - ev) * (1.0 - eg)) * ev * SB(Tv);
}
...
void canopy_rain_interception(double rain_max_loading,
                              double LSAI,
                              double Prain,
                              double *max_storage,
                              double *storage,
...
{
  double load;
  double coeff = 0.25; /*from The Partitioning of Evapotranspiration into
                        Transpiration, Soil Evaporation, and Canopy Evaporation
                        in a GCM: Impacts on Land–Atmosphere Interaction,DAVID
                        M. LAWRENCE, PETER E. THORNTON, KEITH W. OLESON, AND
                        GORDON B. BONAN, JOURNAL OF HYDROMETEOROLOGY,
...
void canopy_snow_interception(double snow_max_loading,
                              double LSAI,
                              double Psnow,
                              double Tc,
                              double v,
                              ...
                              double *drip)
{
  ...
  *max_storage = snow_max_loading * LSAI;
  ...
  load = ((*max_storage) - (*storage)) *
         (1. - exp(-Psnow / (*max_storage)));  // Niu & Yang, 2004
  *storage = (*storage) + load;
  *drip = Psnow - load;
  ...
  unload =
    Fmin(*storage, (*storage) * (Fmax(0.0, Tc + 3.0) / CT + v / CV) * Dt);
...
void update_roughness_veg(double hc,
                          double snowD,
                          double zmu,
                          double zmt,
                          double *z0_ris,
          ...
          long row)
{
  ...
  // root_fraction[] = weighting factor assigned to each layer for
  // transpiration, sum of all root_fraction components gives 1

  long l;
  double z = 0.0;
  double d_corr =
    d * cos(slope);  // slope depth taken as ortogonal to layer boundary
...
void canopy_evapotranspiration(double rbv,
                               double Tv,
                               double Qa,
                               double Pa,
                               double SWin,
  ...
  for (l = 1; l < fl.size(); l++)
    {
      ...
      if (theta[l] >= soil[sy][jfc][l])
        ...
        {
          fl[l] =
            (theta[l] - soil[sy][jwp][l]) / (soil[sy][jfc][l] - soil[sy][jwp][l]);
        }
      else
        {
          fl[l] = 0.0;
...
void veg_transmittance(short stabcorr_incanopy,
                       double /*v*/,
                       double u_star,
                       double u_top,
                       double Hveg,
                       ...
                       double *decay)
{
  ...
  double zm = d0veg + z0veg;  // height above surface at which the canopy
  // energy/momentum sink/source are supposed to
  // occur
  ...
  *rb = Fmin(1.E20, 70.0 * sqrt(Lc / u_veg)) / LSAI;
  ...
  r = (Hveg * exp(*decay) / (d0veg * (*decay))) *
      (exp(-(*decay) * z0soil / Hveg) - exp(-(*decay) * zm / Hveg));
  Ktop = GTConst::ka * u_star * (Hveg - d0veg) / phi_above;
  *rh = Fmin(1.E20, r * d0veg / Ktop +
             2.0 * pow(r, 0.45) /
             (GTConst::ka * u_star));  // 2.0 comes from ln(z0/z0h)
}
