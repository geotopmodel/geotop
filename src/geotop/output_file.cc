...
namespace geotop
{
  namespace input
  {
    ...
    OutputFile::OutputFile(std::string extended_key,
                           double period,
                           long layer,
                           std::string prefix)
    {
      ...
      if (l > 0)
        {
          if (mPrefix.at(l - 1) != '/') mPrefix.push_back('/');
        }
              ...
              if (mVariable == geotop::input::UNKNOWN_VAR)
                {
                  ...
                  lg->logsf(geotop::logger::CRITICAL,
                            "Unknown output variable: '%s'. Modify the geotop.inpts. "
                            "Aborting.",
                            tmp.c_str());
    ...
    std::string OutputFile::getFileName(double dateeur12, long layer)
    {
      ...
      if (mDimension == D1Dp || mDimension == D1Ds)
        ...
        {
          convert_dateeur12_daymonthyearhourmin(dateeur12, &day, &month, &year,
                                                &hour, &min);
          sprintf(buffer, "%.4ld%.2ld%.2ld%.2ld%.2ld", year, month, day, hour,
                  min);
      ...
      if (mPeriod >= 86400) sprintf(buffer, "%ldd", mPeriod / 86400);
    ...
    Variable OutputFile::str2var(std::string v)
    {
      ...
      if (tmp.compare(toLower(getLongString(SOIL_WATER_CONTENT))) == 0)
        return SOIL_WATER_CONTENT;
      if (tmp.compare(toLower(getLongString(SOIL_ICE_CONTENT))) == 0)
        return SOIL_ICE_CONTENT;
      if (tmp.compare(toLower(getLongString(SOIL_WATER_PRESSURE))) == 0)
        return SOIL_WATER_PRESSURE;
      if (tmp.compare(toLower(getLongString(SOIL_TOTAL_PRESSURE))) == 0)
        return SOIL_TOTAL_PRESSURE;
      // if (tmp.compare(toLower(getLongString(SOIL_ET))) == 0) return SOIL_ET;
      if (tmp.compare(toLower(getLongString(SOIL_CAN_RAIN))) == 0)
        return SOIL_CAN_RAIN;
      if (tmp.compare(toLower(getLongString(SOIL_CAN_SNOW))) == 0)
        return SOIL_CAN_SNOW;
      // SNOW MAP VARIABLES
      if (tmp.compare(toLower(getLongString(SNOW_AGE))) == 0) return SNOW_AGE;
      // if (tmp.compare(toLower(getLongString(SNOW_DEPTH))) == 0) return
      // SNOW_DEPTH;
      if (tmp.compare(toLower(getLongString(SNOW_HN))) == 0) return SNOW_HN;
      if (tmp.compare(toLower(getLongString(SNOW_MELTED))) == 0)
        return SNOW_MELTED;
      if (tmp.compare(toLower(getLongString(SNOW_SUBL))) == 0) return SNOW_SUBL;
      if (tmp.compare(toLower(getLongString(SNOW_DURATION))) == 0)
        return SNOW_DURATION;
      // if (tmp.compare(toLower(getLongString(SNOW_CA))) == 0) return SNOW_CA;
      // WATER MAP VARIABLES
      if (tmp.compare(toLower(getLongString(PREC_TOTAL))) == 0)
        return PREC_TOTAL;
      ...
      if (tmp.compare(toLower(getLongString(ENER_SWinb))) == 0)
        return ENER_SWinb;
      ...
      if (tmp.compare(toLower(getLongString(METEO_AIRTEMP))) == 0)
        return METEO_AIRTEMP;
      if (tmp.compare(toLower(getLongString(METEO_WSPEED))) == 0)
        return METEO_WSPEED;
      if (tmp.compare(toLower(getLongString(METEO_WDIR))) == 0)
        return METEO_WDIR;
      if (tmp.compare(toLower(getLongString(METEO_RH))) == 0) return METEO_RH;
      if (tmp.compare(toLower(getLongString(VECTOR_TEST))) == 0)
        return VECTOR_TEST;
    ...
    bool OutputFile::isValidDimension()
    {
      ...
      if (mDimension == UNKNOWN_DIM) return false;
      ...
      switch (mVariable)
        ...
        case SNOW_AGE:
          output = (mDimension == D3D) ? false : true;
          break;
        // case SNOW_DEPTH:// Dzl is a GeoTensor that has to be cumulate in
        // space
        //     output = true;
        //     break;
        case SNOW_HN:
