  ...
  if (snow->type->co[r][c]==2)
    {
      ...
      //add new layer
      if (snow->w_ice->co[snow->lnum->co[r][c]][r][c] > SWEmax_layer *
          (1.+simpl_snow) )
        {
          occurring = 1;
        }
      ...
      if (occurring == 1)
        {
          ...
          snow->w_liq->co[snow->lnum->co[r][c]+1][r][c] =
            snow->w_liq->co[snow->lnum->co[r][c]][r][c] *
            (snow->w_ice->co[snow->lnum->co[r][c]][r][c] - SWEmax_layer) /
            snow->w_ice->co[snow->lnum->co[r][c]][r][c];
          snow->Dzl->co[snow->lnum->co[r][c]+1][r][c] =
            snow->Dzl->co[snow->lnum->co[r][c]][r][c] *
            (snow->w_ice->co[snow->lnum->co[r][c]][r][c] - SWEmax_layer) /
            snow->w_ice->co[snow->lnum->co[r][c]][r][c];
          snow->w_ice->co[snow->lnum->co[r][c]+1][r][c] =
            snow->w_ice->co[snow->lnum->co[r][c]][r][c] - SWEmax_layer;
          snow->T->co[snow->lnum->co[r][c]+1][r][c] =
            snow->T->co[snow->lnum->co[r][c]][r][c];
                       ...
                       long l2, long lres)
{
  ...
  if (snow->Dzl->co[lres][r][c]<0 || snow->w_ice->co[lres][r][c]<0
...
void write_snow(long r, long c, long l, STATEVAR_3D *snow)
{
  ...
  printf("r:%ld c:%ld wice(%ld/%ld):%f wliq(%ld):%f T(%ld):%f Dz(%ld):%f\n",r,c,
         l,snow->lnum->co[r][c],snow->w_ice->co[l][r][c],l,snow->w_liq->co[l][r][c],
                   ...
                   long l2, double Dmin)
{
  ...
  if (snow->Dzl->co[l1][r][c] < Dmin
      && snow->Dzl->co[l2][r][c] > 1.E-5)  //l1 too shallow and takes mass from l2
    {
              ...
              if (snow->w_ice->co[l][r][c] > simpl_snow*par->max_weq_snow)
                {
                  ...
                  if (snow->w_ice->co[l][r][c]/E->ice->co[m] < 1) snow->Dzl->co[l][r][c] *=
                      (snow->w_ice->co[l][r][c]/E->ice->co[m]);

                  //limit on max porosity
                  if (snow->w_ice->co[l][r][c] / (1.E-3*snow->Dzl->co[l][r][c]*rho_w) >
                      par->snow_maxpor)
                    {
                      snow->Dzl->co[l][r][c] = 1.E3 * snow->w_ice->co[l][r][c] /
                  ...
                  if (th>par->Sr*(1.0-thi)) Wdt += Fmin(5.0*pow(Se,3.0)*Dt,
                                                          (th - par->Sr*(1.0-thi)))*snow->Dzl->co[l][r][c]*1.E-3*rho_w;
              ...
              if (glac->w_ice->co[l][r][c] > simpl_snow*par->max_weq_glac)
                {
                  ...
                  if (glac->w_ice->co[l][r][c]/E->ice->co[m] < 1) glac->Dzl->co[l][r][c] *=
