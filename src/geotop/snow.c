...
void find_SCA(STATEVAR_3D *snow, PAR *par, double **Z, double t){
	...
	fprintf(f,",%f,%f,%f",(JDfrom0-par->init_date->co[i_sim])+(i_run-1)*(par->end_date->co[i_sim]-par->init_date->co[i_sim]),JDfrom0,JD);  
...
double interpolate_snow(long r, long c, double h, long max, DOUBLETENSOR *Dz, DOUBLETENSOR *Q, short k){
			...
			if (l == 1) {
				...
				if(u>0){
					if (k==0) {
						...
						q = ( Q->co[l-1][r][c]/Dz->co[l-1][r][c] * (h-z0) + Q->co[l][r][c]/Dz->co[l][r][c] * (z-h) ) / (z - z0);
					}
				}else{
					if (k==0) {
						q = ( Q->co[l-1][r][c] * (z-h) + Q->co[l][r][c] * (h-z0) ) / (z - z0);
					}else {
						q = ( Q->co[l-1][r][c]/Dz->co[l-1][r][c] * (z-h) + Q->co[l][r][c]/Dz->co[l][r][c] * (h-z0) ) / (z - z0);
