                ...
                double Ss)
{
  ...
  psisat =
    (pow((pow(1.0 - i / (s - r), -1.0 / m) - 1.0), 1.0 / n)) * (-1.0 / a);
  if (psi > psisat) sat = 1;
...
// it is the derivative of teta with respect to psi [mm^-1]
{
  ...
  if (psi >= psisat)
    ...
    {
      dteta = (s - r) * (a * m * n) * (pow(-a * psi, n - 1.0)) *
              pow(1.0 + pow(-a * psi, n), -m - 1.0);
    }
...

{
  ...
  k =
    ksat * pow(TETA, v) * (pow((1 - pow((1 - pow(TETA, (1.0 / m))), m)), 2.0));
 ...
 * It is used to calculate the property K at the interface between two
 * consecutive layers K1,D1 and K2,D2 mean=(D1+D2)/(D1/K1+D2/K2) Author: Matteo
 * Dall'Amico, Sept 2011 */
{
...
/******************************************************************************************************************************************/
// double psi_from_theta(double th, double ice, long l, double **pa, double
// pmin){
