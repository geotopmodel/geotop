...
short interpolate_meteo(short flag, double dX, double dY,
                        ...
                        DOUBLEVECTOR *Yst, double **value, long metcod, double **grid, double dn0,
                        short iobsint)
{
  ...
  for (n=1; n<=Xst->nh; n++)
    {
      if ((long)value[n-1][metcod]!=number_absent
          && (long)value[n-1][metcod]!=number_novalue) nstn++;
    }
  ...
  if (nstn>1)
    {
      ...
      barnes_oi(flag, Xpoint, Ypoint, Xst, Yst, xst, yst, var, dn,
...
void barnes_oi(short flag, DOUBLEMATRIX *xpoint, DOUBLEMATRIX *ypoint,
               DOUBLEVECTOR *xstnall, DOUBLEVECTOR *ystnall,
               DOUBLEVECTOR *xstn, DOUBLEVECTOR *ystn, DOUBLEVECTOR *var, double dn,
               double undef,
