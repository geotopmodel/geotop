...
void Turbulence::aero_resistance(double zmu,
                                 double zmt,
                                 double z0,
                                 double d0,
                                 double z0_z0t,
                                 ...
                                 long maxiter)
{
  ...
  if (state_turb == 0)
    ...
    {
      Businger(MO, zmu, zmt, d0, z0, v, 0.5 * T + 0.5 * Ta, T - Ta, Q - Qa,
               z0_z0t, rm, rh, rv, Lobukhov, maxiter);

    }
  else if (state_turb ==
           2)    // catabatic flows - OERLEMANS & GRISOGONO (2002)
    {
      *rm = 1.0 / (v * GTConst::ka * GTConst::ka /
                   (log((zmu - d0) / z0) * log((zmu - d0) / z0)));
      if (p * (-gmT + 0.0098) > 0.0015)
        {
          *rh = 1 / (0.0004 * (Ta - T) *
                     pow(GTConst::GRAVITY / (GTConst::tk * p * (gmT + 0.0098) * 5),
                         0.5));
        }
      else
        {
          *rh = 1 / (0.0004 * (Ta - T) *
                     pow(GTConst::GRAVITY / (GTConst::tk * (0.0015) * 5), 0.5));
        }
      *rv = *rh;
    }
}
...
void Turbulence::turbulent_fluxes(const double &rh,
                                  const double &rv,
                                  const double &P,
                                  const double &Ta,
                                  const double &T,
                                  ...
                                  double &dEdT)
{
  ...
  /*a maximum value of the resistance for the sensible heat flux was introduced
  according to Jordan et el., 1999 (HEAT BUDGET OF SNOW-COVERED SEA ICE AT NORTH
  POLE 4) as Cwindless=0.5 W m^2 K^-1, rh=rho*cp/C=1300/0.5=2600*/
...
double Turbulence::Psim(const double &z)
{
  ...
  psi = 2.0 * log((1.0 + x) / 2.0) + log((1.0 + x * x) / 2.0) - 2.0 * atan(x) +
        0.5 * GTConst::Pi;
...
{
  double psi =
    10.71 + 0.7 * z + 0.75 * (z - 14.28) * exp(-0.35 * z);  // Holstag&De Bruin
...
// void Turbulence::Lewis(double zmu, double zmt, double d0, double z0, double
// z0_z0t, double Ta, double Ts, double v, double *rm, double *rh,
//           double *rv, DOUBLEVECTOR *w)
void Turbulence::Lewis(double zmu,
                       double zmt,
                       double d0,
                       double z0,
                       double z0_z0t,
                       ...
                       GeoVector<double> &w)
{
  ...
  /* CH neutrale [m/s]*/
  Cmn =
    GTConst::ka * GTConst::ka / (log((zmu - d0) / z0) * log((zmu - d0) / z0));
  Chn =
    GTConst::ka * GTConst::ka / (log((zmu - d0) / z0) * log((zmt - d0) / z0t));

  /* calcola la funzione di stabilita' secondo la trattazione semplificata di
   * Garrat, 1992 */
...
double Turbulence::cz(double zmeas,
                      double z0,
                      double d0,
                      double L,
                      double (*unstab)(const double &z),
                      double (*stab)(const double &z))
{
  double c, zeta;
...
double Turbulence::CZ(short state,
                      double zmeas,
                      double z0,
                      double d0,
                      double L,
                      double (*Psi)(const double &z))
{
  double c;
  FILE *f;
...
void Turbulence::Star(short a,
                      ...
                      double *z0v,
                      double (*Psi)(const double &z),
                      double (*roughness)(const double &x,
                                          const double &y,
                                          const double &z))
{
...
void Turbulence::Businger(short a,
                          double zmu,
                          double zmt,
                          double d0,
                          double z0,
  ...
  do
    {
      ...
      // Conductances
      Star(a, zmu, z0, d0, L, 0.0, v, 1.0, 0.0, 1.0, &u_star, &cm, &z0v,
           (&Turbulence::Psim), (&Turbulence::roughT));  // momentum
      if (z0_z0t == 0.0)                                 // rigid surface
        {
          Star(a, zmt, z0, d0, L, u_star, DT, u_star * z0 / 1.4E-5, 1.0, 0.0,
               &T_star, &ch, &z0t, (*Psih), (&Turbulence::roughT));  // heat flux
          Star(a, zmt, z0, d0, L, u_star, DQ, u_star * z0 / 1.4E-5, 1.0, 0.0,
               &Q_star, &cv, &z0q, (*Psih),
               (&Turbulence::roughQ));  // water vapour flux
        }
      else                            // bending surface
        {
          Star(a, zmt, z0, d0, L, u_star, DT, 1.0, 0.0, 1.0 / z0_z0t, &T_star, &ch,
               &z0t, (*Psih), (&Turbulence::roughT));  // heat flux
          Star(a, zmt, z0, d0, L, u_star, DQ, 1.0, 0.0, 1.0 / z0_z0t, &Q_star, &cv,
               &z0q, (*Psih), (&Turbulence::roughQ));  // water vapour flux
        }

      // Obukhov length
      L = -u_star * u_star * (T + GTConst::tk) /
          (GTConst::ka * GTConst::GRAVITY *
           (T_star - 0.61 * Q_star * (T + GTConst::tk)));

      cont++;

    }
  while (fabs(100 * T_star + 100 * u_star + 1000 * Q_star - tol) > 0.01 &&
         cont <= maxiter);
  ...
  if (d0 > zmu || d0 > zmt)
    {
      f = fopen(geotop::common::Variables::FailedRunFile.c_str(), "w");
      fprintf(
        f, "Error:: Displacement height greater than measurement elevations\n");
      fclose(f);
      t_error("Fatal Error! Geotop is closed. See failing report.");
    }
  if (zmu <= z0 || zmt <= z0t || zmt <= z0q)
    {
      f = fopen(geotop::common::Variables::FailedRunFile.c_str(), "w");
      fprintf(f,
              "Error:: Elevation of sensors lower than roughness length: zmu=%f "
              "zmt=%f z0=%f z0t=%f\n",
              zmu, zmt, z0, z0t);
      fclose(f);
      t_error("Fatal Error! Geotop is closed. See failing report.");
...
void Turbulence::find_actual_evaporation_parameters(
  long /*R*/,
  long /*C*/,
  double *alpha,
  double *beta,
  GeoVector<double> &evap_layer,
  double *theta,
  ...
  else
    {
      ...
      if (psi > 10.)    // ponding
      ...
      else      // unsaturated
        {
          ...
          for (l = 1; l < n; l++)
            {
              D = GTConst::D00 * pow((T[l] + GTConst::tk) / GTConst::tk, 2.) *
                  (GTConst::Pa0 / P);  // molecular diffusivity water vapor [mm2/s]
              ...
              if (theta[l] <= soil[sy][jfc][l])
                {
                  hs = 0.5 * (1. - cos(GTConst::Pi * (theta[l] - soil[sy][jres][l]) /
                                       (soil[sy][jfc][l] - soil[sy][jres][l])));
                }
              else
                {
                  hs = 1.;
              ...
              // evap_layer->co[l] = rho*(soil[jsat][l] - theta[l]) * hs * Qsat /
              // r->co[l];
              evap_layer[l] = rho * (soil[sy][jsat][l] - theta[l]) * hs * Qsat / r[l];

              // A += ( (soil[jsat][l] - theta[l]) / (soil[jsat][1] - theta[1]) ) *
              // (rv/r->co[l]) * hs * Qsat;
              A += ((soil[sy][jsat][l] - theta[l]) / (soil[sy][jsat][1] - theta[1])) *
                   (rv / r[l]) * hs * Qsat;
              // B += ( (soil[jsat][l] - theta[l]) / (soil[jsat][1] - theta[1]) ) *
              // (rv/r->co[l]);
              B += ((soil[sy][jsat][l] - theta[l]) / (soil[sy][jsat][1] - theta[1])) *
                   (rv / r[l]);
            }
          ...
          //  evap_layer->co[1] += rho*( theta[1] - soil[jres][1] ) * F * ( Qgsat
          //- Qa ) / rv;
          evap_layer[1] +=
            rho * (theta[1] - soil[sy][jres][1]) * F * (Qgsat - Qa) / rv;

          *beta = (soil[sy][jsat][1] - theta[1]) +
                  (theta[1] - soil[sy][jres][1]) * F -
                  (soil[sy][jsat][1] - theta[1]) / (1. + B);
          *alpha = ((theta[1] - soil[sy][jres][1]) * F +
                    (soil[sy][jsat][1] - theta[1]) * A / (Qgsat * (1. + B))) /
                   (*beta);
        }
    }
}
