...
template <class T>
GeoVector<T>::GeoVector(const size_t &asize)
{
  resize(asize);
  setMetaData();
}
...
template <class T>
GeoVector<T>::GeoVector(const size_t &asize, const T &init)
{
  resize(asize, init);
  setMetaData();
}
...
template <class T>
void GeoVector<T>::setMetaData(const std::string &iname,
                               const std::string &iunit,
                               const size_t &iprecision)
{
  name = iname;
  unit = iunit;
...
template <class T>
void GeoVector<T>::reset(const T &val)
{
...
template <class T>
void GeoVector<T>::reset(const T &val, const T &val_to_omit)
{
  const double data_size = data.size();
  for (size_t ii = 0; ii < data_size; ii++)
    {
      if (data[ii] != val_to_omit) { data[ii] = val; }
    }
}

template <class T>
size_t GeoVector<T>::size() const
{
#if defined(__linux)
  PRINT_FRAME_STACK
#endif
...
template <class T>
void GeoVector<T>::resize(const size_t &asize)
{
  data.resize(asize);
}

template <class T>
void GeoVector<T>::resize(const size_t &asize, const T &init)
{
  resize(asize);
...
template <class T>
inline T &GeoVector<T>::at(const size_t &index)
{
  return data.at(index);
}

template <class T>
inline const T GeoVector<T>::at(const size_t &index) const
{
  return data.at(index);
}

template <class T>
inline T &GeoVector<T>::operator[](const size_t &index)
{
  return data[index];
}

template <class T>
inline const T GeoVector<T>::operator[](const size_t &index) const
{
  return data[index];
}

template <class T>
void GeoVector<T>::clear()
{
  resize(0);
}
...
template <class T>
GeoMatrix<T>::GeoMatrix() : mio::Array2D<T>()
{
  setMetaData();
}

template <class T>
GeoMatrix<T>::GeoMatrix(const size_t &anx, const size_t &any)
  : mio::Array2D<T>(anx, any)
{
  setMetaData();
}

template <class T>
GeoMatrix<T>::GeoMatrix(const size_t &anx, const size_t &any, const T &init)
  : mio::Array2D<T>(anx, any, init)
{
  setMetaData();
}

template <class T>
void GeoMatrix<T>::setMetaData(const std::string &iname,
                               const std::string &iunit,
                               const size_t &iprecision)
{
  name = iname;
...
template <class T>
void GeoMatrix<T>::reset(const T &val)
{
...
template <class T>
void GeoMatrix<T>::reset(const T &val, const T &val_to_omit)
{
  for (unsigned int ii = 0; ii < this->nx; ii++)
    {
      for (unsigned int jj = 0; jj < this->ny; jj++)
...
template <class T>
size_t GeoMatrix<T>::getRows() const
{
#if defined(__linux)
  PRINT_FRAME_STACK
#endif
...
template <class T>
size_t GeoMatrix<T>::getCols() const
{
#if defined(__linux)
  PRINT_FRAME_STACK
#endif
...
template <class T>
GeoTensor<T>::GeoTensor() : mio::Array3D<T>()
{
  setMetaData();
}

template <class T>
GeoTensor<T>::GeoTensor(const size_t &anx, const size_t &any,
                        const size_t &anz)
  : mio::Array3D<T>(anx, any, anz)
{
  setMetaData();
}

template <class T>
GeoTensor<T>::GeoTensor(const size_t &anx,
                        const size_t &any,
                        const size_t &anz,
                        const T &init)
  : mio::Array3D<T>(anx, any, anz, init)
{
  setMetaData();
}

template <class T>
void GeoTensor<T>::setMetaData(const std::string &iname,
                               const std::string &iunit,
                               const size_t &iprecision)
{
  name = iname;
...
template <class T>
void GeoTensor<T>::reset(const T &val)
{
  for (unsigned int ii = 0; ii < this->nx; ii++)
    {
      for (unsigned int jj = 0; jj < this->ny; jj++)
...
template <class T>
void GeoTensor<T>::reset(const T &val, const T &val_to_omit)
{
  for (unsigned int ii = 0; ii < this->nx; ii++)
    {
      for (unsigned int jj = 0; jj < this->ny; jj++)
...
template <class T>
size_t GeoTensor<T>::getDh() const
{
#if defined(__linux)
  PRINT_FRAME_STACK
#endif
...
template <class T>
size_t GeoTensor<T>::getRh() const
{
#if defined(__linux)
  PRINT_FRAME_STACK
#endif
...
template <class T>
size_t GeoTensor<T>::getCh() const
{
#if defined(__linux)
  PRINT_FRAME_STACK
#endif
