	...
	if(Fetch>=300){
		...
		if(Ustar > Usthr){
			...
			if(Nsalt > 0){
				...
				Mpm = 4.0/3.0 * Pi * rho_i * Mpr*pow(Mpr,2.0) *(1.0 + 3.0/Alpha + 2.0/pow(Alpha,2.0));
...
double sublimation(double Z){	
	...
	C = 1.0/(Diff * SvDens * Nuss);
	DmDt = (2.0*Pi * Mpr * SigmaZ)/(Ls*B/A + C);
	Mpm = 1.333 * Pi * rho_i * pow(Mpr,3.0) * (1.0 + 3.0/Alpha + 2.0/pow(Alpha,2.0));  //{Eq. 6.16} {Gamma Dist. Corr.}
