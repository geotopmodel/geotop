...
namespace GTConst
{
  ...
  const double KNe = 0.0;  // Euler method parameter for heat equation (0.5 =
  // Crank Nicholson, 0 = Backward Euler)
  const double LSAIthres = 0.1;  // Minimum LSAI
  const double z_evap =
    100.;  // soil depth responsable for soil evaporation [mm]
  const double z_transp =
    10000.;  // soil depth responsable for canopy transpiration [mm]
  ...
  // STANDARD LAPSE RATES: For the sign, remember that the Lapse Rate gives how
  // a variable decrease with height
  ...
  const double GRAVITY = 9.81;      /* gravity acceleration [m/s2] */
  ...
  const double mu_l =
    0.001787; /* Dynamic viscosity of water at 0 degrees Celsius*/
  ...
  const double Rwv =
    461.495;  // Specific gas constant for water vapor, 461.495 J/(kg·K)
  const double D00 = 21.7;  // molecular diffusivity of water vapor, 21.7 mm2/s
  const double secinday = 86400.0;  // seconds in one day
  const int nl = 1;  // TODO: noori- we use this constant temporarily as a
  // replacement of the fluidturtle vector's ln
}  // namespace GTConst
...
const unsigned int jz0thressoil = jz0 +
                                  1;   /*threshold on snow depth to change
                                                roughness length to snow covered
                                                values in soil area*/
const unsigned int jHveg = jz0thressoil + 1; /*vegetation height*/
const unsigned int jz0thresveg =
  jHveg + 1; /*threshold on snow depth to change roughness length to snow
                covered values in vegetated area*/
...
const unsigned int jvR_nir =
  jvR_vis + 1; /*vegetation in the near infrared spectrum*/
const unsigned int jvT_vis = jvR_nir +
                             1; /*vegetation in the visible spectrum*/
const unsigned int jvT_nir =
  jvT_vis + 1; /*vegetation in the near infrared spectrum*/
const unsigned int jvCh =
  jvT_nir + 1; /*departure of leaf angles from a random distribution (1
                  horizontal, 0 random, -1 vertical)*/
const unsigned int jcd = jvCh +
                         1; /*surface density of canopy [kg/(m2*LSAI)]*/
const unsigned int ja_vis_dry =
  jcd + 1; /*ground albedo in the visible spectrum dry*/
const unsigned int ja_nir_dry =
  ja_vis_dry + 1; /*ground albedo in the near infrared spectrum dry*/
const unsigned int ja_vis_sat =
  ja_nir_dry + 1; /*ground albedo in the visible spectrum saturated*/
const unsigned int ja_nir_sat =
  ja_vis_sat + 1; /*ground albedo in the near infrared spectrum saturated*/
const unsigned int jemg = ja_nir_sat + 1; /*soil emissivity*/
const unsigned int jcm =
  jemg + 1; /*gauckler strickler (1/manning) coefficient*/
const unsigned int jN =
  jcm + 1; /*number of roughness elements (vegetation) per unit surface for
              blowing snow*/
const unsigned int jdv =
  jN + 1; /*diameter of roughness elements (vegetation) [mm]*/
...
const unsigned int jdz0thresveg =
  jdHveg + 1; /*threshold on snow depth to change roughness length to snow
                 covered values in vegetated area*/
...
const unsigned int oprecrain =
  oprecsnow + 1; /*(prec_rain_on_soil+prec_rain_on_snow);*/
...
const unsigned int ominLWin =
  oLWin + 1; /*(epsa_min*5.67E-8*pow(Tpoint+tk,4.0))/(double)n;*/
const unsigned int omaxLWin =
  ominLWin + 1; /*(epsa_max*5.67E-8*pow(Tpoint+tk,4.0))/(double)n;*/
...
const unsigned int otrasp =
  oevapsur + 1; /*Evt*(1000.0/rho_w)*par->Dt; //Etc[mm]*/
const unsigned int owcan_rain =
  otrasp + 1; /*wat->wcan_rain->co[r][c]/(double)n;*/
const unsigned int owcan_snow =
  owcan_rain + 1;                        /*wat->wcan_snow->co[r][c]/(double)n;*/
...
const unsigned int fhorpoint =
  fpointlist +
  1; /*horizon of points for which the simulation is run 1D (point_sim==1)*/
...
const unsigned int fSCA = fglzwriteend + 1; /*file giving the fraction of snow
                                               free areas and corresponding
                                               properties*/
...
const unsigned int fliqsup =
  fliq + 1; /*o. water content in the soil at the surface*/
...
const unsigned int fhsupland =
  ficeav + 1; /*o. water over the surface (mm) maps*/
const unsigned int fhsupch =
  fhsupland + 1; /*o. water over the surface (mm) maps*/
...
const unsigned int fcint =
  fprec + 1; /*o. precipitation intercepted by canopy maps*/
...
const unsigned int fglacsubl = fglacmelt +
                               1; /*o. glacier ice sublimated maps*/
...
const unsigned int fHN = fwtable_dw + 1;        /* HN file TODO mattiu*/
const unsigned int fpnet = fHN + 1;
...
const unsigned int pHg =
  pLE + 1; /*specific day map plots(p.) sensible heat flux*/
...
const unsigned int pth =
  pD + 1; /*p. water content of the most superficial layer*/

// first letter "r" are recovery files

const unsigned int rpsi =
  pth + 1; /*recover file (f.) psi (liquid water pressure)*/
...
const unsigned int rpsich =
  rTv + 1; /*r. psi (liquid water pressure) in channels*/
...
// Points
//****************************************************
const unsigned int ptX = 1;
const unsigned int ptY = ptX + 1;
const unsigned int ptID = ptY + 1;
const unsigned int ptZ = ptID + 1;
