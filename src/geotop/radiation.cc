...
double power_rad(double a, double b){
    return exp(b*log(a));
}
/******************************************************************************************************************************************/
/******************************************************************************************************************************************/
                         ...
                         double Lozone, double a, double b, double rho_g)
{
  ...
  mr = 1./(sin(X)+0.15*(power_rad((3.885+X*180.0/Pi),-1.253)));
  ma = mr*P/Pa0;
  w = 0.493*RH*(exp(26.23-5416.0/(T+tk)))/(T+tk); //cm
  U1 = w*mr;
  U3 = Lozone*mr;
  tau_r = exp(-.0903*power_rad(ma,.84)*(1.+ma-power_rad(ma,1.01)));
  tau_o = 1. - (.1611*U3*power_rad(1.+139.48*U3, -.3035) - .002715*U3/(1.+.044*U3+.0003*U3*U3));
  tau_g = exp(-.0127*power_rad(ma,.26));
  tau_w = 1. - 2.4959*U1/(power_rad(1.+79.034*U1,.6828)+6.385*U1);
  //tau_a from 7.4.11 Iqbal
  tau_a = .12445*a - 0.0162 + (1.003 - .125*a) * exp(-b*ma*(1.089*a + .5123));
  tau_aa = 1. - (1. - w0)*(1. - ma + power_rad(ma, 1.06))*(1. - tau_a);
                        ...
                        double *eps_min)
{
  ...
  if (state==1)
    ...
    {
      *eps_min = (0.601+5.95*0.00001*pvap*exp(1500.0/(T
                                                      +tk)));//Andreas and Ackley, 1982
    ...
    {
      *eps_min = (1.-(1.+46.5*pvap/(T+tk))*exp(-pow(1.2+3.*46.5*pvap/(T+tk),
                                                    0.5))); //Prata 1996

    }
  else if (state==9)
    {
      *eps_min = ( 59.38 + 113.7*pow( (T+tk)/273.16,
                                      6. ) + 96.96*pow((465.*pvap/(T+tk))/25., 0.5) ) / (5.67E-8*pow(T+tk,
                                          4.));//Dilley 1998
