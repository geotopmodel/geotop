...
void sun(double JDfrom0, double *E0, double *Et, double *Delta)
{
  ...
  *E0 = 1.00011 + 0.034221 * cos(Gamma) + 0.00128 * sin(Gamma) +
...
double SolarHeight(double JD, double latitude, double Delta, double dh)
{
  ...
  sine = sin(latitude) * sin(Delta) +
         cos(latitude) * cos(Delta) * cos(GTConst::omega * (12 - h));
  if (sine > 1) sine = 1.;
  if (sine < -1) sine = -1.;
...
{
  return SolarHeight_(JD, (double *)others);
}
  ...
  if (h <= 12)
    {
      if (alpha == GTConst::Pi / 2.0)    // zenith
        ...
        {
          cosine = (sin(alpha) * sin(latitude) - sin(Delta)) /
                   (cos(alpha) * cos(latitude));
...
{
  return SolarAzimuth_(JD, (double *)others);
}
  ...
  if (height > 0)
    {
      ...
      return atm_transmittance(Fmax(height, asin(0.05)), P, RH, T, Lozone, alpha,
                               beta, albedo) *
             Fmax(0.0, cos(slope) * sin(height) +
                  sin(slope) * cos(height) * cos(-aspect + dir));
    }
  else
    {
      return 0.0;
...
{
  return TauatmCosinc(JD, (double *)others);
}
  ...
  if (height > 0)
    {
      return atm_transmittance(Fmax(height, asin(0.05)), P, RH, T, Lozone, alpha,
                               beta, albedo) *
             Fmax(sin(height), 0.05);  // Matteo, August 2015 corrected 0.5 with
      // 0.0 according to UZH
...
{
  return TauatmSinalpha(JD, (double *)others);
}
...
double Cosinc(double JD, double *others)
{
  ...
  if (alpha > 0)
    {
      return Fmax(0.0, cos(slope) * sin(alpha) +
                  sin(slope) * cos(alpha) * cos(-aspect + direction));
    }
  else
    {
      return 0.0;
...
{
  return Cosinc(JD, (double *)others);
}
...
{
  return Sinalpha(JD, (double *)others);
}
...
{
  return Tauatm(JD, (double *)others);
}
...
void shortwave_radiation(double JDbeg,
                         double JDend,
                         double *others,
                         double sin_alpha,
                         double E0,
                         ...
                         short *SWb_yes)
{
  ...
  tau_atm = adaptiveSimpsons2(Tauatm_, others, JDbeg, JDend, 1.E-6, 20) /
            (JDend - JDbeg);
  ...
  *tau_atm_sin_alpha =
    adaptiveSimpsons2(TauatmSinalpha_, others, JDbeg, JDend, 1.E-6, 20) /
    (JDend - JDbeg);
  //*tau_atm_sin_alpha = tau_atm * sin_alpha;

  *SWd = GTConst::Isc * E0 * tau_cloud * (*tau_atm_sin_alpha) * sky * kd +
         (1. - sky) * SWrefl_surr;
  ...
  if (shadow == 1)
    ...
    {
      cos_inc = adaptiveSimpsons2(Cosinc_, others, JDbeg, JDend, 1.E-6, 20) /
                (JDend - JDbeg);
      // cos_inc = Cosinc( 0.5*(JDbeg+JDend), others);
      tau_atm_cos_inc =
        adaptiveSimpsons2(TauatmCosinc_, others, JDbeg, JDend, 1.E-8, 20) /
        (JDend - JDbeg);
  ...
  *cos_inc_bd = kd * sin_alpha + (1. - kd) * cos_inc;
  ...
  printf("tauatm:%f tau_cloud:%f kd:%f tausin:%f sky:%f SWrefl:%f  \n", tau_atm,
         tau_cloud, kd, *tau_atm_sin_alpha, sky, SWrefl_surr);
  printf("sh:%d cosinc:%f taucos:%f cosincbd:%f SWb:%f SWd:%f \n", shadow,
         cos_inc, tau_atm_cos_inc, *cos_inc_bd, *SWb, *SWd);
#endif
}
...
double diff2glob(double a)
{
  ...
  if (a < 0.22)
    ...
    {
      k = 0.9511 - 0.1604 * a + 4.388 * pow(a, 2.0) - 16.638 * pow(a, 3.0) +
          12.336 * pow(a, 4.0);
    }
  else
    {
...
double atm_transmittance(double X, double P, double RH, double T, double Lozone,
double a, double b, double rho_g){

    //X = angle of the sun above the horizon [rad]
    //P = pressure [mbar]
    //RH = relative humidity [0-1]
    //T = air temperature [C]
    //from Mayers and Dale, Predicting Daily Insolation with Hourly Cloud Height
and Coverage, 1983, pg 537, Journal of Climate and Applied Meteorology
...
// double atm_transmittance_iqbal(double X, double P, double RH, double T,
// double Lozone, double a, double b, double rho_g){
double atm_transmittance(double X,
                         double P,
                         double RH,
                         double T,
                         double Lozone,
                         ...
                         double rho_g)
{
  ...
  ma = mr * P / GTConst::Pa0;
  w = 0.493 * RH * (exp(26.23 - 5416.0 / (T + GTConst::tk))) /
      (T + GTConst::tk);  // cm
  U1 = w * mr;
  U3 = Lozone * mr;
  tau_r = exp(-.0903 * pow(ma, .84) * (1. + ma - pow(ma, 1.01)));
  tau_o = 1. - (.1611 * U3 * pow(1. + 139.48 * U3, -.3035) -
                .002715 * U3 / (1. + .044 * U3 + .0003 * U3 * U3));
  tau_g = exp(-.0127 * pow(ma, .26));
  tau_w = 1. - 2.4959 * U1 / (pow(1. + 79.034 * U1, .6828) + 6.385 * U1);
  // tau_a from 7.4.11 Iqbal
  tau_a = .12445 * a - 0.0162 +
          (1.003 - .125 * a) * exp(-b * ma * (1.089 * a + .5123));
  ...
  tau_atm_dr = .79 * tau_o * tau_g * tau_w * tau_aa * .5 * (1. - tau_r) /
               (1. - ma + pow(ma, 1.02));
  tau_atm_da = .79 * tau_o * tau_g * tau_w * tau_aa * Fc *
               (1. - tau_a / tau_aa) / (1. - ma + pow(ma, 1.02));
...
void longwave_radiation(short state,
                        double pvap,
                        double /*RH*/,
                        double T,
                        double k1,
                        ...
                        double *eps_min)
{
  ...
  if (state == 1)
    {
      *eps_min =
        1.24 * pow((pvap / (T + GTConst::tk)), 1. / 7.);  // Brutsaert, 1975
  ...
  else if (state == 2)
    {
      *eps_min =
        1.08 *
        (1.0 - exp(-pow(pvap, (T + GTConst::tk) / 2016.0)));  // Satterlund, 1979

    }
  else if (state == 3)
    {
      *eps_min = (0.7 + 5.95 * 0.00001 * pvap *
                  exp(1500 / (T + GTConst::tk)));  // Idso(1981)

    }
  else if (state == 4)
    {
      *eps_min = (0.7 + 5.95 * 0.00001 * pvap * exp(1500 / (T + GTConst::tk)));
      *eps_min = -0.792 + 3.161 * (*eps_min) -
                 1.573 * (*eps_min) * (*eps_min);  // IDSO + HODGES
    ...
    {
      *eps_min =
        (0.601 + 5.95 * 0.00001 * pvap *
         exp(1500.0 / (T + GTConst::tk)));  // Andreas and Ackley, 1982

    }
  else if (state == 7)
    {
      *eps_min = (0.23 + k1 * pow((pvap * 100.) / (T + GTConst::tk),
                                  1. / k2));  // Konzelmann (1994)

    }
  else if (state == 8)
    {
      *eps_min = (1. - (1. + 46.5 * pvap / (T + GTConst::tk)) *
                  exp(-pow(1.2 + 3. * 46.5 * pvap / (T + GTConst::tk),
                           0.5)));  // Prata 1996

    }
  else if (state == 9)
    {
      *eps_min = (59.38 + 113.7 * pow((T + GTConst::tk) / 273.16, 6.) +
                  96.96 * pow((465. * pvap / (T + GTConst::tk)) / 25., 0.5)) /
...
void rad_snow_absorption(long r,
                         long c,
                         GeoVector<double> &frac,
                         double R,
                         Statevar3D *snow)
{
  long l, m;
      ...
      for (l = snow->lnum[r][c]; l >= 1; l--)
        {
          ...
          rho = (snow->w_ice[l][r][c] + snow->w_liq[l][r][c]) /
...
/******************************************************************************************************************************************/
double cloud_transmittance(double JDbeg,
                           double JDend,
                           double lat,
                           double Delta,
                           double dh,
                           ...
                           double albedo)
{
  ...
  if (tau_atm_sin_alpha > minTauAtmSinAlpha)    // matteo added sept 2015 to
    {
      // avoid problems in calculating
      // Tau at sunset when
      // solarHeight is very low
      if ((long)SWd != geotop::input::gDoubleAbsent &&
          (long)SWd != geotop::input::gDoubleNoValue &&
          (long)SWb != geotop::input::gDoubleAbsent &&
          (long)SWb != geotop::input::gDoubleNoValue)
        {
          if (SWb + SWd > 0 && SWd > 0)
            {
              kd = SWd / (Fmax(0., SWb) + SWd);
              tau = (SWd - (1. - sky) * SWrefl_surr) /
                    (GTConst::Isc * E0 * tau_atm_sin_alpha * sky * kd);
            }

        }
      else if ((long)SW != geotop::input::gDoubleAbsent &&
               (long)SW != geotop::input::gDoubleNoValue)
        {
          kd = 0.2;
          tau_atm = adaptiveSimpsons2(Tauatm_, others, JDbeg, JDend, 1.E-6, 20) /
                    (JDend - JDbeg);
          sin_alpha =
            adaptiveSimpsons2(Sinalpha_, others, JDbeg, JDend, 1.E-6, 20) /
            (JDend - JDbeg);
          j = 0;
          ...
          do
            {
              ...
              tau = (SW - (1. - sky) * SWrefl_surr) /
                    (GTConst::Isc * E0 * tau_atm_sin_alpha * ((1 - kd) + sky * kd));
              if (tau > 1)
                tau = 1.0;  // commented after check against UZH (Matteo, August 2015)
...
double find_tau_cloud_station(double JDbeg,
                              double JDend,
                              long station_number,
                              Meteo *met,
                              ...
                              double albedo)
{
  ...
  double swDirect = geotop::input::gDoubleNoValue,
         swDiffuse = geotop::input::gDoubleNoValue;

  const MeteoData &current =
    vec_meteo.at(station_number - 1);  // MeteoIO starts counting at 0
  ...
  if ((current(MeteoData::TA) != IOUtils::nodata) &&
      (current(MeteoData::RH) != IOUtils::nodata) &&
      (current(MeteoData::P) != IOUtils::nodata))
    {
      tdew = tDew(current(MeteoData::TA) - GTConst::tk,
                  current(MeteoData::RH) * 100.0, current(MeteoData::P) / 100.0);
    }
  ...
  if (current(MeteoData::RH) != IOUtils::nodata)
    ...
    {
      if ((current(MeteoData::TA) != IOUtils::nodata) &&
          (tdew != geotop::input::gDoubleNoValue))
        {
          RH = RHfromTdew(current(MeteoData::TA) - GTConst::tk, tdew,
                          current.meta.position.getAltitude());
        }
      else
        {
          RH = 0.4;
  ...
  c = cloud_transmittance(
        JDbeg, JDend, current.meta.position.getLat() * GTConst::Pi / 180., Delta,
        (current.meta.position.getLon() * GTConst::Pi / 180. -
         ST * GTConst::Pi / 12. + Et) /
        GTConst::omega,
        RH, T, P, swDiffuse, swDirect, current(MeteoData::ISWR), E0,
        met->st->sky[station_number], SWrefl_surr, Lozone, alpha, beta, albedo);

  return c;
}

// double find_tau_cloud_station_meteodistr(double JDbeg, double JDend, long i,
// Meteo *met,
//            double Delta, double E0, double Et, double ST, double
//SWrefl_surr)

double find_tau_cloud_station_meteodistr(double JDbeg,
                                         double JDend,
                                         long i,
                                         Meteo *met,
                                         double Delta,
                                         ...
                                         double albedo)
{
  ...
  // relative humidity
  if ((long)met->var[i - 1][iRh] != geotop::input::gDoubleNoValue &&
      (long)met->var[i - 1][iRh] != geotop::input::gDoubleAbsent)
    {
      RH = met->var[i - 1][iRh] / 100.;
    }
  else
    {
      if ((long)met->var[i - 1][iT] != geotop::input::gDoubleAbsent &&
          (long)met->var[i - 1][iT] != geotop::input::gDoubleNoValue &&
          (long)met->var[i - 1][iTdew] != geotop::input::gDoubleAbsent &&
          (long)met->var[i - 1][iTdew] != geotop::input::gDoubleNoValue)
        {
          RH =
            RHfromTdew(met->var[i - 1][iT], met->var[i - 1][iTdew], met->st->Z[i]);
        }
      else
        {
          RH = 0.4;
  ...
  c = cloud_transmittance(
        JDbeg, JDend, met->st->lat[i] * GTConst::Pi / 180., Delta,
        (met->st->lon[i] * GTConst::Pi / 180. - ST * GTConst::Pi / 12. + Et) /
        GTConst::omega,
        RH, T, P, met->var[i - 1][iSWd], met->var[i - 1][iSWb],
        met->var[i - 1][iSW], E0, met->st->sky[i], SWrefl_surr, Lozone, alpha, beta,
        albedo);
...
short shadows_point(double **hor,
                    long n,
                    double alpha,
                    double azimuth,
                    double tol_mount,
...
/** routine that tells you whether a point is in shadow or not, depending on the
 *solar azimuth, elevation and horizon file at that point
 ...
 *double tol_mount: tolerance over a mountaneaus horizon to have a reliable
 *cloud datum (degree) double tol_flat: tolerance over a mountaneaus horizon to
 *have a reliable cloud datum (degree) Output: shad: 1=the point is in shadow, 0
 *the point is in sun
 */
{
  ...
  if (iend > ibeg)
    ...
    {
      w = (azimuth - (hor[ibeg][0] - 360.0)) /
          (hor[iend][0] - (hor[ibeg][0] - 360.0));
    }

  horiz_H = w * hor[iend][1] +
            (1.0 - w) * hor[ibeg][1];  // horizon elevation at a particular time
...
void shadow_haiden(GeoMatrix<double> &Z,
                   double alpha,
                   double direction,
                   GeoMatrix<short> &SH)

/** @Author: Thomas Haiden, Year: 16 june 2003
 *  @brief Function that calculates if each pixel (matrix of shadows) is in
 * shadow or at sun given a solar elevation angle and a solar azimuth. Inputs:
 * top    elevation DTM (m) alpha:    solar elevation angle (radiants)
 *      direction:  solar azimuth angle (from N clockwise, radiants)
 *      point: flag indicating whether the simulation is a point (=1) or a
 * distributed simulation Outputs:shadow:   shadows matrix (1 shadow, 0 sun)
 *  imported by Matteo Dall'Amico on August 2009. Authorization: see email of
 * David Whiteman on 16 june 2011 revised by Stefano Endrizzi on May 2011.
 */
{
          ...
          for (l = 0; l < nl; l++)
            {
              ...
              while ((SH[r][c] == 0) && (kk > 0) && (kk < nk - 1) && (ll > 0) &&
                     (ll < nl - 1))
                {
                  ...
                  if (fabs(y - (GDY * kk + 0.5 * GDY)) < GDY)
                    {
                      ...
                      z2 = Z[rr][cc];
                      ztopo =
                        z1 + (z2 - z1) * (yp - (GDY * kk + 0.5 * GDY)) / (oriy * GDY);
                    }
                  else
                    {
                      ...
                      z2 = Z[rr][cc];
                      ztopo =
                        z1 + (z2 - z1) * (xp - (GDX * ll + 0.5 * GDX)) / (orix * GDX);
                    }
                  zray = zray + q * sz;
...
double find_albedo(double dry_albedo,
                   double sat_albedo,
                   double wat_content,
                   double residual_wc,
                   double saturated_wc)
...
void find_actual_cloudiness_meteoio(
  double *tau_cloud,
  double *tau_cloud_av,
  short *tau_cloud_yes,
...
void find_actual_cloudiness(double *tau_cloud,
                            double *tau_cloud_av,
                            short *tau_cloud_yes,
                            short *tau_cloud_av_yes,
                            int /*meteo_stat_num*/,
                            ...
                            double albedo)
{
  // SWdata = flag -> 0= no radiation data measured, 1=beam and diff measured,
  // 2=global measured
  short SWdata;
  double tc;

  if ((long)met->var[met->nstsrad - 1][iSWb] != geotop::input::gDoubleAbsent &&
      (long)met->var[met->nstsrad - 1][iSWd] != geotop::input::gDoubleAbsent)
    {
      if ((long)met->var[met->nstsrad - 1][iSWb] !=
          geotop::input::gDoubleNoValue &&
          (long)met->var[met->nstsrad - 1][iSWd] !=
          geotop::input::gDoubleNoValue)
        {
          SWdata = 2;
        }
  ...
  else if ((long)met->var[met->nstsrad - 1][iSW] !=
           geotop::input::gDoubleAbsent)
    {
      if ((long)met->var[met->nstsrad - 1][iSW] !=
          geotop::input::gDoubleNoValue)
        {
          SWdata = 1;
        }
  ...
  if (SWdata > 0)
    {
      tc = find_tau_cloud_station_meteodistr(JDb, JDe, met->nstsrad, met, Delta,
                                             E0, Et, ST, SWrefl_surr, Lozone,
                                             alpha, beta, albedo);
      if ((long)tc != geotop::input::gDoubleNoValue)
        {
          *tau_cloud_yes = 1;
  ...
  if ((long)met->var[met->nstcloud - 1][iC] != geotop::input::gDoubleAbsent &&
      (long)met->var[met->nstcloud - 1][iC] != geotop::input::gDoubleNoValue)
    {
  ...
  else if ((long)met->var[met->nstcloud - 1][itauC] !=
           geotop::input::gDoubleAbsent &&
           (long)met->var[met->nstcloud - 1][itauC] !=
           geotop::input::gDoubleNoValue)
    {
...
double find_cloudfactor(double Tair,
                        double RH,
                        double Z,
                        double T_lapse_rate,
