 ...
 
{
    ...
    short a, success, added_JDfrom0=0, added_wind_xy=0, added_wind_dir=0, added_cloud=0, added_Tdew=0, added_RH=0, added_Pint=0, old=0, recovered=0;
    ...
    if(!argv[1]){
        ...
        struct stat s;
        int err = stat(WORKING_DIRECTORY, &s);
        if(-1 == err) {
            printf("Error: directory %s does not exist.\nExiting!", WORKING_DIRECTORY);
            exit(1);
        } else {
            if(!S_ISDIR(s.st_mode)) {
                /* exists but is no dir */
                printf("Error: %s exists but it's not a directory.\nExiting!", WORKING_DIRECTORY);
                exit(1);
            }
        }
    }
    ...
    //printf("Riccardo Rigon and his research group are acknowledged as Geotop 1.225-15  uses most of their modelling achievements.\n");
    //printf("John Pomeroy is acknowledged as he freely provided the Prairie Blowing Snow Model Code.\n");
    //printf("Glen Liston and Kelly Elder are acknowledged as they freely provided their Micromet code, from which the routines that distribute wind-air temperature-relative humidity-precipitation in Geotop 1.225-15  are derived.\n");
    ...
    //fprintf(flog,"Riccardo Rigon and his research group are acknowledged as Geotop 1.225-15  uses most of their modelling achievements.\n");
    //fprintf(flog,"John Pomeroy is acknowledged as he freely provided the Prairie Blowing Snow Model Code.\n");
    //fprintf(flog,"Glen Liston and Kelly Elder are acknowledged as they freely provided their Micromet code, from which the routines that distribute wind-air temperature-relative humidity-precipitation in Geotop 1.225-15  are derived.\n");
    ...
    if(par->state_pixel == 1){
        ...
        if(strcmp(files[fsatz], string_novalue) != 0) sl->satratio = new_doublematrix(par->rc->nrh, Nl);
...
//Channel network (in top->pixel_type)
    ...
    if(flag == 1){
        ...
        for(r=1;r<=top->Z0->nrh;r++){
            for(c=1;c<=top->Z0->nch;c++){
                if((long)land->LC->co[r][c]!=number_novalue){
                    if(top->pixel_type->co[r][c]!=0 && top->pixel_type->co[r][c]!=1 && top->pixel_type->co[r][c]!=2 && top->pixel_type->co[r][c]!=10 && top->pixel_type->co[r][c]!=11 && top->pixel_type->co[r][c]!=12 && top->pixel_type->co[r][c]!=-1){
                        f = fopen(FailedRunFile, "w");
                        fprintf(f, "Error: Only the following values are admitted in the network map: -1, 0, 1, 2, 10\n");
                        fclose(f);
    ...
    for(r=1;r<=top->Z0->nrh;r++){
        for(c=1;c<=top->Z0->nch;c++){
            if(top->is_on_border->co[r][c]==1){
                if (top->pixel_type->co[r][c] == -1 || top->pixel_type->co[r][c] == 1 || top->pixel_type->co[r][c] == 2 || top->pixel_type->co[r][c] == 11 || top->pixel_type->co[r][c] == 12) cont ++;
            }
        }
    }
    ...
    if (cont > 0) {
        ...
        for(r=1;r<=top->Z0->nrh;r++){
            for(c=1;c<=top->Z0->nch;c++){
                if(top->is_on_border->co[r][c]==1){
                    if (top->pixel_type->co[r][c] == -1 || top->pixel_type->co[r][c] == 1 || top->pixel_type->co[r][c] == 2 || top->pixel_type->co[r][c] == 11 || top->pixel_type->co[r][c] == 12){
                        cont ++;
...
void read_optionsfile_point(PAR *par, TOPO *top, LAND *land, SOIL *sl, TIMES *times, INIT_TOOLS *IT, FILE *flog){
    ...
    for(i=1;i<=par->chkpt->nrh;i++){ 
        ...
        if((long)par->chkpt->co[i][ptSY]==number_novalue) par->chkpt->co[i][ptSY]=(double)par->soil_type_land_default;
