...
#ifdef USE_DOUBLE_PRECISION_OUTPUT
        if(V->wsnow[j]<0) printf("Error 1 wcansnow:%12g %ld %ld\n",V->wsnow[j],r,c);
#else
    ...
    *SWupabove_v += (1.0-fc)*( SWdiff*(0.5*avis_d+0.5*anir_d) + SWbeam*(0.5*avis_b+0.5*anir_b) );
    ...
    do{
        ...
        sux=SolvePointEnergyBalance(surface, Tdirichlet,
                                    ...
                                    fc, A->L->vegpar[jdLSAI], A->L->vegpar[jddecay0],
	    ...
	    double *Locc, double *LWup_ab_v, long *lpb, double *dUsl)
{
    ...
    double dH_dT, dE_dT, EB, dEB_dT=0.0, EB0, Tg, Tg0, psim0, psi0, Qg0, Tv0, dWcsn=0.0, dWcrn=0.0, rh_g, rv_g;
	...
	if (surfacebalance == 1){    
		...
		// in this routine there are many variable not used to actually computing something out of them. We could get rid of them.. 
    ...
    for(l=1;l<=n;l++){
        ...
        if (l>1)
		{
            ...
            if (l-1 > ns+ng) { // if the layer (l) is <= num_of_snow_layers(ns) + num_of_glacies(ng) 
			...
			if (l <= ns+ng) { // if the layer (l) is <= num_of_snow_layers(ns) + num_of_glacies(ng) 
    ...
    do{
        ...
        for(l=1;l<=n;l++){
            ...
            if(l<=ns+ng){//snow
				...
				{
					C1 += GTConst::rho_w*GTConst::Lf*(GTConst::Lf/(GTConst::GRAVITY*GTConst::tk)*1.E3)*
					dteta_dpsi(Psif(egy->Temp[l]), 0.0, sl->pa[sy][jsat][m],sl->pa[sy][jres][m], sl->pa[sy][ja][m],sl->pa[sy][jns][m],1-1/sl->pa[sy][jns][m],GTConst::PsiMin, 0.0);
	...
	//		 if(dirichlet == 1) egy->Fenergy[sur] -= ( (Tgd-egy->Temp[sur])*(1.-KNe)*kub1 + (Tgd-egy->T0[sur])*KNe*kub0 ) / (egy->Dlayer[1]/2.);
