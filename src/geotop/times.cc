...
/*==================================================================================================================*/
/*==================================================================================================================*/
/*Calculates the fractional Julian day from 01/01/0000 0:00 (set at Julian Day =
 1) given the year and Julian Day of the year (from 1-Jan 0:00 of the year).
 This is the reference used in Matlab*/
  ...
  if (year < 0)
    {
      ...
      fprintf(f,
              "Error:: Not Possible to handle dates with year earlier than 0\n");
      fclose(f);
 ...
 * @param[out] min pointer to a long that will hold the minutes (must not be
 * NULL)
 */
void convert_JDandYear_daymonthhourmin(double JD,
                                       long year,
                                       long *d,
                                       long *m,
                                       long *h,
                                       long *min)
{
  ...
  long days_in_months_in_leap[] = {0,  31, 29, 31, 30, 31, 30,
                                   31, 31, 30, 31, 30, 31
                                  };
  long days_in_months_in_non_leap[] = {0,  31, 28, 31, 30, 31, 30,
                                       31, 31, 30, 31, 30, 31
                                      };
  long *dim = NULL;
  ...
  if (d != NULL && m != NULL && h != NULL && min != NULL)
    {
      ...
      *h = (int)floor(((((double)1440.0) * frac - (double)*min) / (double)60.0) +
                      0.5);
    }
  else
    {
 ...
 */
double convert_daymonthyearhourmin_JD(long d,
                                      long m,
                                      long y,
                                      long h,
                                      long min)
{
  ...
  if (min < 0 || min > 59 || h < 0 || h > 23 || m < 1 || m > 12 || d < 1 ||
      d > dim[m])
    {
...
void convert_dateeur12_daymonthyearhourmin(double date,
                                           long *day,
                                           long *month,
                                           long *year,
                                           long *hour,
                                           long *min)
{
  ...
  *min = floor(date / 1.E0 - (*day) * 1.E10 - (*month) * 1.E8 - (*year) * 1.E4 -
               (*hour) * 1.E2);

  if (*day < 1 || *day > 31 || *month < 1 || *month > 12 || *year < 1700 ||
      *year > 2900 || *hour < 0 || *hour > 23 || *min < 0 || *min > 59)
    {
      f = fopen(geotop::common::Variables::FailedRunFile.c_str(), "w");
...
double convert_daymonthyearhourmin_dateeur12(long day,
                                             long month,
                                             long year,
                                             long hour,
                                             long min)
