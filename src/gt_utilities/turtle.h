...
#define isDynamic                                          \
  1 /*This number will be used to mark a dynamic allocates \
quantity */
#define MAXBUFFERSIZE                                       \
  50000 /* Many functions uses a buffer to allocate data. A \
safe habit is to limit its size. */

#define MAXLABELSIZE 1024 /* The same as above */

#define BUFFERINCREMENT                                    \
  256 /*Buffers memory is increases when necessary of this \
storage capability till MAXBUFFERSIZE*/

#define LABELINCREMENT 64 /*The same as above */

#define NR_END                                                \
  1 /* Required by the Numerical Recipes' allocation routines \
*/
...
#define PRINT 1
#define NOPRINT                                             \
  0 /*With the above PRINT is used to set a printint option \
in some functions */

#define NL                                                   \
  1 /* Numerical Recipes allocation routines allow to have   \
