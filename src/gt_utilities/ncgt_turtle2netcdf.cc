...
//	// if (status!=NC_NOERR) ERROR_MESSAGE(status,function_name,"nc_enddef");ncid);
	 ...
	 /* Write the pretend data to the file. Although netCDF supports
	    * reading and writing subsets of data, in this case we write all
	    * the data in one operation. */
	status=nc_put_var_double(ncid,dvar,&(m->co[m->nrl][m->ncl]));
	if (status!=NC_NOERR) ERROR_MESSAGE(status,function_name,"nc_put_var_double");
...
*/
	...
	// if (status!=NC_NOERR) ERROR_MESSAGE(status,function_name,"nc_enddef");ncid);
	 ...
	 /* Write the pretend data to the file. Although netCDF supports
	    * reading and writing subsets of data, in this case we write all
	    * the data in one operation. */
	status=nc_put_var_float(ncid,dvar,&(v->co[v->nl]));
...
*/
	...
	// if (status!=NC_NOERR) ERROR_MESSAGE(status,function_name,"nc_enddef");ncid);
	 ...
	 /* Write the pretend data to the file. Although netCDF supports
	    * reading and writing subsets of data, in this case we write all
	    * the data in one operation. */
	status=nc_put_var_int(ncid,dvar,&(v->co[v->nl]));
...
*/
	...
	// if (status!=NC_NOERR) ERROR_MESSAGE(status,function_name,"nc_enddef");ncid);
	 ...
	 /* Write the pretend data to the file. Although netCDF supports
	    * reading and writing subsets of data, in this case we write all
	    * the data in one operation. */
	status=nc_put_var_long(ncid,dvar,&(v->co[v->nl]));
...
*/
	...
	// if (status!=NC_NOERR) ERROR_MESSAGE(status,function_name,"nc_enddef");ncid);
	 ...
	 /* Write the pretend data to the file. Although netCDF supports
	    * reading and writing subsets of data, in this case we write all
	    * the data in one operation. */
	status=nc_put_var_short(ncid,dvar,&(v->co[v->nl]));
...
//	if (status!=NC_NOERR) ERROR_MESSAGE(status,function_name,"nc_open");
	...
	// if (status!=NC_NOERR) ERROR_MESSAGE(status,function_name,"nc_enddef");ncid);
	 ...
	 /* Write the pretend data to the file. Although netCDF supports
	    * reading and writing subsets of data, in this case we write all
	    * the data in one operation. */
	status=nc_put_var_float(ncid,dvar,&(m->co[m->nrl][m->ncl]));
	if (status!=NC_NOERR) ERROR_MESSAGE(status,function_name,"nc_put_var_float");
...
//	if (status!=NC_NOERR) ERROR_MESSAGE(status,function_name,"nc_open");
	...
	// if (status!=NC_NOERR) ERROR_MESSAGE(status,function_name,"nc_enddef");ncid);
	 ...
	 /* Write the pretend data to the file. Although netCDF supports
	    * reading and writing subsets of data, in this case we write all
	    * the data in one operation. */
	status=nc_put_var_short(ncid,dvar,&(m->co[m->nrl][m->ncl]));
	if (status!=NC_NOERR) ERROR_MESSAGE(status,function_name,"nc_put_var_short");
...
//	if (status!=NC_NOERR) ERROR_MESSAGE(status,function_name,"nc_open");
	...
	// if (status!=NC_NOERR) ERROR_MESSAGE(status,function_name,"nc_enddef");ncid);
	 ...
	 /* Write the pretend data to the file. Although netCDF supports
	    * reading and writing subsets of data, in this case we write all
	    * the data in one operation. */
	status=nc_put_var_int(ncid,dvar,&(m->co[m->nrl][m->ncl]));
	if (status!=NC_NOERR) ERROR_MESSAGE(status,function_name,"nc_put_var_int");
...
//	if (status!=NC_NOERR) ERROR_MESSAGE(status,function_name,"nc_open");
	...
	// if (status!=NC_NOERR) ERROR_MESSAGE(status,function_name,"nc_enddef");ncid);
	 ...
	 /* Write the pretend data to the file. Although netCDF supports
	    * reading and writing subsets of data, in this case we write all
	    * the data in one operation. */
	status=nc_put_var_long(ncid,dvar,&(m->co[m->nrl][m->ncl]));
	if (status!=NC_NOERR) ERROR_MESSAGE(status,function_name,"nc_put_var_long");
...
//	if (status!=NC_NOERR) ERROR_MESSAGE(status,function_name,"nc_open");
	...
	// if (status!=NC_NOERR) ERROR_MESSAGE(status,function_name,"nc_enddef");ncid);
	 ...
	 /* Write the pretend data to the file. Although netCDF supports
	    * reading and writing subsets of data, in this case we write all
	    * the data in one operation. */
	status=nc_put_var_double(ncid,dvar,&(dt->co[dt->nrl][dt->ncl][dt->ndl]));
	if (status!=NC_NOERR) ERROR_MESSAGE(status,function_name,"nc_put_var_double");
...
//	// if (status!=NC_NOERR) ERROR_MESSAGE(status,function_name,"nc_enddef");ncid);
	...
	std::cout<<"m->nrh " <<m->nrh <<"m->nch " <<m->nch << " m-nrl=" << m->nrl << " m-ncl=" << m->ncl <<std::endl;
	stop_execution();
	 ...
	 * reading and writing subsets of data, in this case we write all
	 * the data in one operation. */
	printf("\nstart[0]=%ld, start[1]=%ld, start[2]=%ld, count[0]=%ld, count[1]=%ld, count[2]=%ld",start[0], start[1], start[2],count[0], count[1], count[2]);stop_execution();
...
int ncgt_put_doublematrix_vs_time(GeoMatrix<double>& m, long k, int ncid, const char *dimension_t,  const char *dimension_x, const char *dimension_y){
	 ...
	 *
	 */
	const char *function_name="ncgt_putgeomatrix_vs_time";
	int status=NC_NOERR;
	int dimid_t,dimid_x,dimid_y; /* pointer to the dimension of the doublematrix;*/
	int dvar; /* pointer to the variable of the doublematrix */
	int ndim=3;
...
//	if (status!=NC_NOERR) NC_GEOTOP_ERROR_MESSAGE(status,function_name,"nc_redef");
	...
	if (status!=NC_NOERR) {
		//status=nc_def_dim(ncid,dimension_y,m->nrh, &dimid_y);
		status=nc_def_dim(ncid,dimension_y, m.getCols(), &dimid_y);
		if (status!=NC_NOERR) ERROR_MESSAGE(status,function_name,"nc_inq_dim");
	}
	/* dimension_x */
	status=nc_inq_dimid(ncid,dimension_x,&dimid_x);
	if (status!=NC_NOERR) {
		//status=nc_def_dim(ncid,dimension_x,m->nch, &dimid_x);
		status=nc_def_dim(ncid,dimension_x,m.getCols(), &dimid_x);
		if (status!=NC_NOERR) ERROR_MESSAGE(status,function_name,"nc_inq_dim");
	}
	   /* The dim array is used to pass the dimids of the dimensions of
	           the netCDF variables. Both of the netCDF variables we are
...
//	// if (status!=NC_NOERR) ERROR_MESSAGE(status,function_name,"nc_enddef");ncid);
	...
	count[0]=1;// time dimension
	//count[1]=m->nrh;// y dimension
	count[1]=m.getRows();// y dimension
	//count[2]=m->nch;// x dimension
	count[2]=m.getCols();// x dimension
	//The indices are relative to 0, so for example,the first data value of a variable would have index (0, 0, ... , 0).
	start[0]=k;// time dimension
	start[1]=0;//m->nrl;//EV// y dimension
	start[2]=0;//m->ncl;//EV// y dimension
...
//status=nc_put_vara_double(ncid,dvar,start,count,&(m->co[m->nrl][m->ncl]));
status=nc_put_vara_double(ncid,dvar,start,count,&(v->co[v->nl]));
if (status!=NC_NOERR) ERROR_MESSAGE(status,function_name,"nc_put_vara_double");
...
//	// if (status!=NC_NOERR) ERROR_MESSAGE(status,function_name,"nc_enddef");ncid);
	...
	status=nc_put_vara_double(ncid,dvar,start,count,&(t->co[t->nrl][t->ncl][t->ndl]));
	if (status!=NC_NOERR) ERROR_MESSAGE(status,function_name,"nc_put_vara_double");
...
int ncgt_put_doubletensor_vs_time(GeoTensor<double>&t, long k, int ncid, const char *dimension_t,  const char *dimension_x, const char *dimension_y, const char *dimension_z)
{
	...
	status=nc_inq_dimid(ncid,dimension_z,&dimid_z);
	if (status!=NC_NOERR) {
		status=nc_def_dim(ncid,dimension_z,t.getDh(), &dimid_z);
		if (status!=NC_NOERR) ERROR_MESSAGE(status,function_name,"nc_inq_dim");
	}
	/* dimension_y */
	status=nc_inq_dimid(ncid,dimension_y,&dimid_y);
	if (status!=NC_NOERR) {
		status=nc_def_dim(ncid,dimension_y,t.getRh(), &dimid_y);
		if (status!=NC_NOERR) ERROR_MESSAGE(status,function_name,"nc_inq_dim");
	}
	/* dimension_x */
	status=nc_inq_dimid(ncid,dimension_x,&dimid_x);
	if (status!=NC_NOERR) {
		status=nc_def_dim(ncid,dimension_x,t.getCh(), &dimid_x);
		if (status!=NC_NOERR) ERROR_MESSAGE(status,function_name,"nc_inq_dim");
	}
...
//	// if (status!=NC_NOERR) ERROR_MESSAGE(status,function_name,"nc_enddef");ncid);
	...
	start[2]=0;
	start[3]=0;
	//printf("count[0]=%d,count[1]=%d,count[2]=%d,count[3]=%d,start[0]=%d,start[1]=%d,start[2]=%d,start[3]=%d",count[0],count[1],count[2],count[3],start[0],start[1],start[2],start[3]);stop_execution();
	/* Write the pretend data to the file. Although netCDF supports
	 * reading and writing subsets of data, in this case we write all
	 * the data in one operation. */

	//status=nc_put_vara_double(ncid,dvar,start,count,&(t->co[t->nrl][t->ncl][t->ndl]));
	status=nc_put_vara_double(ncid,dvar,start,count,&(t(1,1,1)));
	if (status!=NC_NOERR) ERROR_MESSAGE(status,function_name,"nc_put_vara_double");
...
int ncgt_put_rotate180_y_doubletensor_vs_time(DOUBLETENSOR *t, long k, int ncid, const char *dimension_t,  const char *dimension_x, const char *dimension_y, const char *dimension_z)
{
	...
	char *function_name="ncgt_put_rotate180_y_doubletensor_vs_time";


	l=rotate180_y_doubletensor(t);
	if (l!=0) printf("Error in %s: rotate180_y_doubletensor() did not work correctly \n",function_name);
	l=ncgt_put_doubletensor_vs_time(t,k,ncid,dimension_t,dimension_x,dimension_y,dimension_z);
	if (l!=0) printf("Error in %s: ncgt_put_doubletensor() did not work correctly  \n",function_name);
	l=rotate180_y_doubletensor(t);
	if (l!=0) printf("Error in %s: rotate180_y_doubletensor() did not work correctly  \n",function_name);
...
{
	/*!
	 *\param t - (GeoTensor ) variable to be written in the NetCDF
	 *\param ncid (int) - pointer to the netCDF file
	 *\param k        - (long) number of the level (0 based) at which the xyz map is printed
	 *\param dimension_t - (char *) name of the t dimension (number of times: UNLIMITED)
	 *\param dimension_x - (char *) name of the x dimension (number of column)
...
//	M=new_doublematrix(dt->ndh,rc->nrh);
	M=new_doublematrix(dt->ndh,rc->getRows()-1);
...
//	M=new_doublematrix(dt->ndh,rc->nrh);
	M=new_doublematrix(dt.getDh()-1, rc->getRows()-1);
	M->name=join_strings((char *)dt.name.c_str(), suffix);
	//std::cout <<"dt.getDh() "<<dt.getDh()<< " rc->getRows()"<< rc->getRows() <<std::endl;
	//stop_execution();
	for(id=M->ncl;id<=M->nch;id++) {
		for(l=M->nrl;l<=M->nrh;l++) {
		//	r=rc->co[id][1];
...
//	M=new_doublevector(rc->nrh);
	M=new_doublevector(rc->getRows()-1);
	M->name=join_strings((char *)dt->name,suffix);
