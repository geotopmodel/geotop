...
void find_min_max(GeoMatrix<double> &M,
                  long novalue,
                  double *max,
                  double *min)
{
...
// adapted to GEOMatrix/Vector to get rid of fluidturtle datastructure: to
// double check if things are ok SC 13.12.2016
...
void sky_view_factor(GeoMatrix<double> &sky,
                     ...
                     long novalue)
{
  size_t i = 0, j = 0, t = 0, m = 0, n = 0, p = 0, q = 0, h = 0, k = 0, r = 0,
         s = 0;  // counters
  ...
  GeoVector<double> v;  // vector with the view factor of the current pixel for
  // one of the N parts
  GeoVector<double> vv;  // vector with the minimum view factor of the current
  // pixel for one of the N parts
  ...
  // ALLOCATION: please note that I allocate 1 element more for columns and
  // row.. " TODO  double checks
      ...
      for (j = 1; j <= 2 * nc - 1; j++)
        {
          if (i <= nr && j < nc)
            {
              alfa[i][j] = 3.0 / 2.0 * GTConst::Pi +
                           atan(((nr - i) * UV->U[1]) / ((nc - j) * UV->U[1]));
            }
          if (i > nr && j <= nc)
            {
              alfa[i][j] =
                GTConst::Pi + atan(((nc - j) * UV->U[1]) / ((i - nr) * UV->U[1]));
            }
          if (i >= nr && j > nc)
            {
              alfa[i][j] = GTConst::Pi / 2.0 +
                           atan(((i - nr) * UV->U[1]) / ((j - nc) * UV->U[1]));
            }
          if (i < nr && j >= nc)
            {
  ...
  for (i = 1; i <= nr; i++)
    {
                  ...
                  for (k = n; k <= q; k++)
                    {
                      for (t = 1; t <= N; t++)
                        {
                          if (alfa[h][k] >= (t - 1) * deltateta &&
                              alfa[h][k] < t * deltateta)
                            {
                              r = h - m + 1;
                              s = k - n + 1;
                              long r_i = r - i;  // I need to convert to a signed variable
                              // since i can be greater than r
                              long s_j = s - j;  // same here
                              if (convess[r][s] == 1 &&
                                  sqrt(pow((r_i), 2) + pow((s_j), 2)) != 0)
                                {
                                  vv[t] =
                                    1 -
                                    sin(atan((input[r][s] - input[i][j]) /
                                             (sqrt(pow((r_i), 2) + pow((s_j), 2)) * UV->U[1])));
                                  if (vv[t] < v[t]) { v[t] = vv[t]; }
                                }
                              break;
                            }
                        }
      ...
      printf(
        "Percentage of the calculation of the sky view factor matrix: %5.2f%%\n",
                ...
                long n)
{
  ...
  // the +1 above is needed to prevent a memory corruptions on glibc on Linux
  // boxes (S.C. 16.12.2016)
  nr = Zin.getRows();
  nc = Zin.getCols();
          ...
          if ((long)Zin[r][c] != novalue)
            {
              ...
              /*order_values(values, cnt);
               ...
               Zout->co[r][c] =
               0.5*(values->co[(long)(0.5*(cnt))]+values->co[(long)(0.5*(cnt)+1)]);
               }*/
          ...
          if ((long)topo[r][c] != undef)
            {
              ...
              if (R1 >= 1 && R1 <= nr && R2 >= 1 && R2 <= nr && C1 >= 1 && C1 <= nc &&
                  C2 >= 1 && C2 <= nc)
                {
                  //  if((long)topo->co[R1][C1]!=undef &&
                  //(long)topo->co[R2][C2]!=undef){
                  if ((long)topo[R1][C1] != undef && (long)topo[R2][C2] != undef)
                    {
                      //  if( (topo->co[R2][C2] - topo->co[r][c]) * (topo->co[r][c] -
                      //topo->co[R1][C1]) < 0){
                      if ((topo[R2][C2] - topo[r][c]) * (topo[r][c] - topo[R1][C1]) < 0)
                        {
                          //  if( fabs(topo->co[r][c] - topo->co[R1][C1]) >
                          //fabs(topo->co[R2][C2] - topo->co[r][c]) ){
                          if (fabs(topo[r][c] - topo[R1][C1]) >
                              fabs(topo[R2][C2] - topo[r][c]))
                            {
                      ...
                      //  }else if((long)topo->co[R1][C1]==undef &&
                      //(long)topo->co[R2][C2]!=undef){
                    }
                  else if ((long)topo[R1][C1] == undef &&
                           (long)topo[R2][C2] != undef)
                    {
                      //  a += (topo->co[R2][C2] - topo->co[r][c]) / delta;
                      a += (topo[R2][C2] - topo[r][c]) / delta;
                      //  }else if((long)topo->co[R1][C1]!=undef &&
              ...
              // Some compilers will not allow dzdx and dzdy to both be 0.0 in the
              // atan2 computation.  if (fabs(dzdx->co[r][c])<1.E-10) dzdx->co[r][c]
              // = 1.E-10;
      ...
      for (c = 1; c < nc; c++)
        {
          if ((long)topo[r][c] != undef)
            {
              M[r][c] = (180. / GTConst::Pi) *
                        atan(pow(pow(dzdx[r][c], 2.0) + pow(dzdy[r][c], 2.0), 0.5));
            }
          else
            {
              M[r][c] = (double)undef;
          ...
          for (c = 1; c < nc; c++)
            {
              if ((long)topo[r][c] != undef)
                {
                  M[r][c] = (180. / GTConst::Pi) *
                            (3.0 / 2.0 * GTConst::Pi - atan2(dzdy[r][c], dzdx[r][c]));
